...
def _forward_benchmark_replay_realized_trace_final_y_and_tangent(
    execution_context,
    carry0,
    carry0_dot,
    trace,
):
    """Replay the frozen adaptive trace with explicit tangent updates.

    Accepted attempts update both primal and tangent. Rejected attempts update
    only frozen primal/controller bookkeeping; their tangent contribution is
    exactly the incoming tangent.
    """
    ...
    def _accepted_attempt(carry_value, xs):
        ...
        accepted_y = project_flat(attempt_result.trial_y) if project_flat is not None else attempt_result.trial_y
        return dataclasses.replace(
...
def _forward_benchmark_adaptive_final_y_realized_schedule_jvp(
    ...
    _replay_primal_out, tangent_out = _forward_benchmark_replay_realized_trace_final_y_and_tangent(
        execution_context,
        carry0,
        carry0_dot,
        rollout.trace,
...
def _frozen_replay_nonfinite_debug(
    ...
    *,
    ...
    trace_state_finite = np.ones(active_mask.shape[0], dtype=bool)
    if state_finite_mask.shape[0] == active_mask.shape[0]:
        trace_state_finite = state_finite_mask.copy()
        active_state_finite = trace_state_finite[active_mask]
        ...
        trace_state_finite[active_indices] = active_state_finite
    else:
        raise ValueError(
