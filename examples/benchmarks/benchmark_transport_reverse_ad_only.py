...
def _report_path(parameter_name: str) -> Path:
    ...
    return outdir / "transport_reverse_ad_only_summary.json"
...
def main() -> None:
    ...
    gradient_rev = jax.jacrev(objective_fn)(jnp.asarray(baseline_value))
    ...
    print(
        f"[autodiff-gate] mode=transport_reverse_ad_only parameter={args.parameter} "
    ...
    for label, value in zip(OBJECTIVE_LABELS, grad_np.tolist()):
        print(f"  - {label}: rev={float(value):.6e}")
