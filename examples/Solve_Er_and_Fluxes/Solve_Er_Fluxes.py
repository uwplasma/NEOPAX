...
Output_Path=os.path.join(current_path,'./examples/Solve_Er_and_Fluxes/')
...
Lij,Gamma,Q,Upar=NEOPAX.get_Neoclassical_Fluxes(final_species,grid,field,database)


J_boots_corr=(-Upar_mom[:,0]+Upar_mom[:,1]+Upar_mom[:,2])*elementary_charge
Er_final=sol.ys[0][-1,:]

Flux_e_corr=Gamma_mom[:,0]
Flux_i_corr=Gamma_mom[:,1]+Gamma_mom[:,2]


#Bootstrap current can be compared against the sum for all species of the SFINCS quantity FSABFlow (after multiplying the normalizations v_bar*n_bar*B_bar )
J_boots=(-Upar[0]+Upar[1]+Upar[2])*elementary_charge
#Particle fluxes can be compared against the SFINCS quantity particleFlux_vd_psiHat (after multiplying the normalizations v_bar*n_bar/R_bar)
Flux_e=Gamma[0]
Flux_i=Gamma[1]+Gamma[2]

file=h5.File('Er_Test.h5','w')
file['rho']=field.rho_grid
file['Er']=Er_final
file['Jboots_corr']=J_boots_corr
file['Jboots']=J_boots
file['Flux_I']=Flux_i
file['Flux_e']=Flux_e
file['Flux_I_corr']=Flux_i_corr
file['Flux_e_corr']=Flux_e_corr
file.close()
...
plt.xlabel('$\\rho$')
plt.ylabel('$E_r [kV/m] $')
ax.plot(field.rho_grid,sol.ys[0][0,:],label='initial')
...
ax.plot(field.rho_grid,(-Upar[0]+Upar[1]+Upar[2])*elementary_charge, label='JAX, no corr')
plt.legend()
...
ax.plot(field.rho_grid,(-Upar[0])*elementary_charge, label='JAX, no corr')
plt.legend()
...
ax.plot(field.rho_grid,(Upar[1]+Upar[2])*elementary_charge, label='JAX, no corr')
plt.legend()
plt.savefig(os.path.join(Output_Path,'J_Bootstrap_with_momentum_IONS.pdf'))
plt.close()


#Plot Gamma ions and electrons
fig,ax=plt.subplots(dpi=120)
plt.ylabel('$\\Gamma$')
plt.xlabel('$\\rho$')
ax.plot(field.rho_grid,Flux_e, label='electrons, with corr')
ax.plot(field.rho_grid,Flux_e, label='electrons, without corr')
ax.plot(field.rho_grid,Flux_i_corr, label='ions, with corr')
ax.plot(field.rho_grid,Flux_i, label='ions, without corr')
plt.legend()
plt.savefig(os.path.join(Output_Path,'Fluxes.pdf'))
plt.close()
