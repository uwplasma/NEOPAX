...
def write_geometry_artifacts(input_obj, label: str) -> Path:
    ...
    input_path = artifact_dir / f"input.QI_neopax_database_finitebeta_transition_bootstrap_{label}"
    input_obj.to_indata(input_path)
    print(f"wrote {input_path}")
    eq = vmex_opt.solve_equilibrium(input_obj)
    wout_path = vj.write_wout(
        artifact_dir / f"wout_QI_neopax_database_finitebeta_transition_bootstrap_{label}.nc",
        eq.wout,
    )
...
def write_outputs(initial_input, optimized_input, initial_problem, initial_x, final_problem, final_x) -> None:
    ...
    optimized_path = OUT_DIR / "input.QI_neopax_database_finitebeta_transition_bootstrap_optimized"
...
def main() -> int:
    ...
    for max_mode in MAX_MODE_SCHEDULE:
        print(f"\n===== finite-beta database QI + Er transition + bootstrap, max_mode={max_mode}, grid=({args.database_n_theta},{args.database_n_phi},{args.database_n_xi}), beta={INCLUDE_BETA}, dmerc={INCLUDE_DMERC}, J_backend={QI_MAXJ_BACKEND} =====", flush=True)
        problem = build_problem(config, current_input, max_mode, args, physical_pitches=frozen_physical_pitches)
        ...
        current_input = OUT_DIR / f"input.QI_neopax_database_finitebeta_transition_bootstrap_stage_m{max_mode}"
