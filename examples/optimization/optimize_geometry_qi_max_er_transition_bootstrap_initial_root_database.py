#!/usr/bin/env python
"""Standalone database-default QI + Er-transition + bootstrap optimization."""
...
OUT_DIR = ROOT / "outputs" / "geometry_qi_max_er_transition_bootstrap_initial_root_database_optimization"
...
# Correspond to optimize_geometry_qi_max_er_transition_bootstrap_initial_root.py.
...
GEOMETRY_ARTIFACT_STEM = "QI_neopax_database_transition_bootstrap"
...
def save_bootstrap_current_profile(problem, x, out_dir: Path, label: str, *, profiles=None) -> None:
    """Save bootstrap current independently of whether its penalty is active."""
...
def main() -> int:
    ...
    for max_mode in (MAX_MODE_SCHEDULE if not np.isscalar(MAX_MODE_SCHEDULE) else (MAX_MODE_SCHEDULE,)):
        print(f"\n===== database QI + Er transition + bootstrap, max_mode={max_mode}, grid=({args.database_n_theta},{args.database_n_phi},{args.database_n_xi}), J_backend={QI_MAXJ_BACKEND} =====", flush=True)
        ...
        current_input = OUT_DIR / f"input.QI_neopax_database_transition_bootstrap_stage_m{max_mode}"
    ...
    optimized_input.to_indata(OUT_DIR / "input.QI_neopax_database_transition_bootstrap_optimized")
