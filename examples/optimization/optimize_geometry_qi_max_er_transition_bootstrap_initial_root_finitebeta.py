#!/usr/bin/env python
"""Standalone finite-beta realtime-Lij QI + Er-transition + bootstrap optimization.
...
OUT_DIR = ROOT / "outputs" / "geometry_qi_max_er_transition_bootstrap_initial_root_finitebeta_optimization"
...
def bootstrap_penalty_value(value):
    return jnp.maximum(value - BOOTSTRAP_LIMIT_SCALED, 0.0)
...
def build_problem(vmec_input, max_mode: int):
    return opt.geometry_initial_er_root_only_least_squares_problem(
        TRANSPORT_CONFIG, active_terms(), vmec_input=vmec_input, max_mode=max_mode, include_profiles=False,
        families=GEOMETRY_FAMILIES, scale_mode=SCALE_MODE, ess_alpha=ESS_ALPHA, mboz=QI_MBOZ, nboz=QI_NBOZ,
        surfaces=tuple(float(item) for item in SURFACES),
        geometry_solver_device=SOLVER_DEVICE, device=SOLVER_DEVICE,
...
def write_geometry_artifacts(input_obj, label: str) -> Path:
    ...
    input_path = artifact_dir / f"input.QI_neopax_realtime_finitebeta_transition_bootstrap_{label}"
    input_obj.to_indata(input_path)
    print(f"wrote {input_path}")
    eq = vmex_opt.solve_equilibrium(input_obj)
    wout_path = vj.write_wout(
        artifact_dir / f"wout_QI_neopax_realtime_finitebeta_transition_bootstrap_{label}.nc",
        eq.wout,
    )
...
def write_outputs(initial_input, optimized_input, initial_problem, initial_x, final_problem, final_x) -> None:
    ...
    optimized_path = OUT_DIR / "input.QI_neopax_realtime_finitebeta_transition_bootstrap_optimized"
...
def main() -> int:
    ...
    for max_mode in MAX_MODE_SCHEDULE:
        print(f"\n===== finite-beta realtime-Lij QI + Er transition + bootstrap, max_mode={max_mode}, beta={INCLUDE_BETA}, dmerc={INCLUDE_DMERC} =====", flush=True)
        ...
        current_input = OUT_DIR / f"input.QI_neopax_realtime_finitebeta_transition_bootstrap_stage_m{max_mode}"
