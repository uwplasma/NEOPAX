...
DATABASE_N_THETA, DATABASE_N_PHI, DATABASE_N_XI = 25, 31, 64
...
def main():
    ...
    for max_mode in MAX_MODE_SCHEDULE:
        ...
        optimized_input = problem.input_from_scaled_parameters(result.x); current_input = OUT_DIR / f'input.QI_neopax_database_finitebeta_transition_stage_m{max_mode}'; optimized_input.to_indata(current_input); final_problem, final_result = problem, result
    ...
    initial_input.to_indata(OUT_DIR / SEED_INPUT.name); optimized_input.to_indata(OUT_DIR / 'input.QI_neopax_database_finitebeta_transition_optimized'); plot_finite_beta_scalars(initial_values, final_values)
