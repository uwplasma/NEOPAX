#!/usr/bin/env python
"""Standalone vacuum QI + database full-transport Er optimization.
...
OUT_DIR = ROOT / "outputs" / "geometry_qi_max_er_transition_bootstrap_initial_root_database_full_transport_optimization"
...
REVERSE_SEGMENT_LENGTH = 250
MAX_REVERSE_ACCEPTED_STEPS = 1000
...
def write_outputs(optimized_input, initial_input, *, config, out_dir, seed_input, make_initial_plots):
    ...
    optimized_input_path = out_dir / "input.QI_neopax_database_full_transport_optimized"
...
def main() -> int:
    ...
    for max_mode in max_mode_schedule:
        print(
            "\n===== vacuum database QI + max-Er/root/bootstrap full transport "
        ...
        problem = opt.GeometryInputSavingProblem(
            ...
            filename_prefix="input.QI_neopax_database_full_transport_eval",
        )
        ...
        report(f"QI + database full-transport stage {max_mode}", problem, x)
        optimized_input = problem.input_from_scaled_parameters(x)
        stage_input = out_dir / f"input.QI_neopax_database_full_transport_stage_m{max_mode}"
