#!/usr/bin/env python
"""Standalone finite-beta realtime-Lij QI + ambipolar-Er-transition optimization.
...
OUT_DIR = ROOT / "outputs" / "geometry_qi_max_er_transition_initial_root_finitebeta_optimization"
...
def write_geometry_artifacts(input_obj, label: str) -> Path:
    ...
    input_path = artifact_dir / f"input.QI_neopax_realtime_finitebeta_transition_{label}"
    input_obj.to_indata(input_path)
    print(f"wrote {input_path}")
    eq = vmex_opt.solve_equilibrium(input_obj)
    wout_path = vj.write_wout(
        artifact_dir / f"wout_QI_neopax_realtime_finitebeta_transition_{label}.nc",
        eq.wout,
    )
...
def write_outputs(initial_input, optimized_input, initial_problem, initial_x, final_problem, final_x) -> None:
    ...
    optimized_path = OUT_DIR / "input.QI_neopax_realtime_finitebeta_transition_optimized"
...
def main() -> int:
    ...
    for max_mode in MAX_MODE_SCHEDULE:
        print(f"\n===== finite-beta realtime-Lij QI + Er transition, max_mode={max_mode}, beta={INCLUDE_BETA}, dmerc={INCLUDE_DMERC} =====", flush=True)
        ...
        current_input = OUT_DIR / f"input.QI_neopax_realtime_finitebeta_transition_stage_m{max_mode}"
