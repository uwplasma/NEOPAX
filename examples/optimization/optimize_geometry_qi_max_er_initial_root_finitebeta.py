#!/usr/bin/env python
"""Standalone finite-beta realtime-Lij QI + maximum-Er optimization.
...
OUT_DIR = ROOT / "outputs" / "geometry_qi_max_er_initial_root_finitebeta_optimization"
...
NFEV, FTOL, XTOL, SOLVER_DEVICE = 15, 1.0e-6, 1.0e-10, "default"
...
def write_geometry_artifacts(input_obj, label: str) -> Path:
    ...
    input_path = artifact_dir / f"input.QI_neopax_realtime_finitebeta_max_er_{label}"
    input_obj.to_indata(input_path)
    print(f"wrote {input_path}")
    eq = vmex_opt.solve_equilibrium(input_obj)
    wout_path = vj.write_wout(
        artifact_dir / f"wout_QI_neopax_realtime_finitebeta_max_er_{label}.nc",
        eq.wout,
    )
...
def write_outputs(initial_input, optimized_input, initial_problem, initial_x, final_problem, final_x) -> None:
    ...
    optimized_path = OUT_DIR / "input.QI_neopax_realtime_finitebeta_max_er_optimized"
...
def main() -> int:
    ...
    for max_mode in MAX_MODE_SCHEDULE:
        ...
        current_input = OUT_DIR / f"input.QI_neopax_realtime_finitebeta_max_er_stage_m{max_mode}"
