...
from . import _reverse_ad_transport as _transport_reverse
from . import _transport_solvers as _transport_solvers
...
GEOMETRY_FULL_AD_OBJECTIVE_ALIASES: Mapping[str, str] = {
    ...
    "vmec_dmerc_stability_softmax": "vmec_dmerc_stability_softmax",
...
def _database_initial_root_to_unfolded_support_bars(
    *,
    fixed_runtime,
    support,
    pre_root_state,
    ...
    This is the exact database-root portion preceding the recorded scan
    transpose.  The caller owns that one current-scan fold, so this helper
    intentionally returns ``{geometry, database}`` bars unchanged.
    """
    ...
    if profile_specs:
        ...
        profile_matrix = jnp.zeros(
            (objective_count, 0), dtype=jnp.asarray(er_profile).dtype
        )
    ...
    return profile_matrix, support_bars


def _database_selected_root_direct_cotangents(
    *,
    config: Mapping[str, object],
    fixed_runtime,
    support,
    ...
    This is the direct half of the database root boundary.  Together with
    :func:`_database_initial_root_to_unfolded_support_bars`, it contains all
    work that will later become the optimization-only JIT candidate.  It does
    not rebuild a live scan or consume a recorded scan transpose.
    """
...
def build_database_initial_root_experiment_stage(
    *,
    ...
    database runtime, and returns unfurled bars. The recorded live scan is
...
def _database_full_transport_initial_er_bars(rooted_state, *, runtime, reverse_setup):
    """Use benchmark segment kernels to obtain the transport bar entering Er."""

    objective_count = len(_transport_reverse.TRANSPORT_REVERSE_OBJECTIVE_LABELS)

    def _carry_from_state(state):
        return _transport_reverse.reverse_initial_carry_from_state_with_static_setup(
    ...
    carry0, state_pullback = jax.vjp(_carry_from_state, rooted_state)
    final_y, residuals = (
        _transport_reverse._reverse_adaptive_final_y_realized_schedule_vjp_fwd(
    ...
    *_, segment_starts, final_carry, segment_arrays = residuals
    if segment_starts is None or final_carry is None or segment_arrays is None:
        raise ValueError("Full-transport root composition requires fixed segments.")

    def _objective_vector(value):
        final_state = reverse_setup.prepared_rollout.physics_context.unpack_flat(value)
        return jnp.stack(
            tuple(
                _transport_reverse.objective_scalar_by_index(final_state, runtime, row)
                for row in range(objective_count)
            )
        )

    values, objective_pullback = jax.vjp(_objective_vector, final_y)
    final_y_bars = jax.vmap(lambda row: objective_pullback(row)[0])(
        jnp.eye(objective_count, dtype=values.dtype)
    )

    def _batched_zero(tree):
        zero = _transport_reverse._reverse_align_tangent_tree_to_primal(
            reverse_setup.execution_context, None, tree
        )
    ...
    bars = _transport_reverse._reverse_reduced_cotangent(
        reverse_setup.execution_context,
        y=final_y_bars,
        lagged_response_cache=_batched_zero(final_carry.lagged_response_cache),
        lagged_reference_y=jnp.zeros(
            (objective_count,) + final_carry.lagged_reference_y.shape,
            dtype=final_carry.lagged_reference_y.dtype,
        ),
    )
    cotangent_mode = str(
        getattr(
            reverse_setup.execution_context.physics_context,
            "reverse_stage_cotangent_mode",
            "full",
        )
    ).strip().lower()
    ...
    segment_count = int(jax.tree_util.tree_leaves(segment_arrays)[0].shape[0])
    for segment_index in range(segment_count - 1, -1, -1):
        bars = _transport_solvers._radau_segment_reduced_cotangent_bwd_batched_call(
            reverse_setup.execution_context,
            cotangent_mode,
            bars,
            _row(segment_starts, segment_index),
            _row(segment_arrays, segment_index),
        )
    ...
    def _carry_bar(reduced_bar):
        return dataclasses.replace(
            jax.tree_util.tree_map(_zero_tangent, carry0),
            y=reduced_bar.y,
            lagged_response_cache=reduced_bar.lagged_response_cache,
            lagged_reference_y=reduced_bar.lagged_reference_y,
        )

    state_bars = jax.vmap(lambda bar: state_pullback(_carry_bar(bar))[0])(bars)
    return dataclasses.replace(
        jax.tree_util.tree_map(jnp.zeros_like, state_bars), Er=state_bars.Er
    )


def database_full_transport_optimization_table_result_builder(
    *,
    table_context: RealtimeGeometryTransportReverseTableContext,
    geometry_context,
    baseline_geometry_deltas,
    database_root_stage: DatabaseInitialRootExperimentStage,
    n_r: int,
    n_theta: int,
    n_zeta: int,
    n_xi: int,
    ...
    reverse_segment_length: int | str | None,
    reverse_stage_adjoint_solve_mode: str,
    reverse_rhs_transpose_mode: str,
    reverse_stage_cotangent_mode: str,
    reverse_step_bwd_mode: str,
    ...
    max_reverse_accepted_steps: int | None,
    progress_label: str | None = None,
) -> TransportReverseTableResultBuilder:
    """Compose the accepted root boundary with the unchanged transport builder."""

    baseline_profiles = jnp.asarray(
        table_context.baseline_values[
            : len(_transport_reverse.TRANSPORT_REVERSE_PROFILE_PARAMETER_ORDER)
        ]
    )
    labels = tuple(_transport_reverse.TRANSPORT_REVERSE_OBJECTIVE_LABELS)

    def _builder(objective_names, parameter_set, options):
        if parameter_set.profile_specs:
            raise NotImplementedError(
                "The database full-transport root bridge is geometry-only."
            )
        opts = {} if options is None else dict(options)
        raw_solve = opts.get("raw_block_solve")
        if raw_solve is None:
            raise ValueError("The database full-transport bridge requires a VMEC solve.")
        profiles = jnp.asarray(
            opts.get("profile_values", baseline_profiles), dtype=baseline_profiles.dtype
        )

        live_runtime, live_state = build_runtime_context_for_vmec_state(
            dict(table_context.config),
            geometry_context,
            raw_solve.state,
            n_r=int(opts.get("n_r", n_r)),
        )
        database_segment, scan_owner = split_recorded_ntx_database_runtime(live_runtime)
        fixed_runtime = database_segment.runtime
        support = {
            "geometry": fixed_runtime.geometry,
            "database": scan_owner.runtime_scan.database,
        }

        database_root_stage.initialize_for_live_runtime(live_runtime)
        adapter = database_root_stage.payload_adapter
        selected_root = database_root_stage.selected_root
        root_pullback = database_root_stage.root_pullback
        if adapter is None or selected_root is None or root_pullback is None:
            raise RuntimeError("The database root stage did not initialize.")
        geometry_leaves, database_leaves = adapter.dynamic_leaves(support)
        pre_root_state = _transport_reverse.initial_state_for_parameter_vector_compact(
            profiles,
            baseline_state=live_state,
            profile_cfg=table_context.profile_cfg,
            geometry=fixed_runtime.geometry,
            number_species=fixed_runtime.species.number_species,
            config=table_context.config,
            initial_er_root_ad="off",
        )
        er_profile, finite_mask = selected_root(
            pre_root_state, geometry_leaves, database_leaves
        )
        rooted_state = dataclasses.replace(pre_root_state, Er=er_profile)

        active_context = dataclasses.replace(
            table_context,
            baseline_runtime=live_runtime,
            baseline_state=rooted_state,
        )
        benchmark_builder = (
            _transport_reverse.internal_realtime_geometry_transport_reverse_table_result_builder(
                ...
                reverse_rhs_transpose_mode=reverse_rhs_transpose_mode,
                reverse_stage_cotangent_mode=reverse_stage_cotangent_mode,
                reverse_step_bwd_mode=reverse_step_bwd_mode,
                reverse_stage_adjoint_memory_mode=reverse_stage_adjoint_memory_mode,
                reverse_stage_adjoint_iter_maxiter=reverse_stage_adjoint_iter_maxiter,
                reverse_stage_adjoint_iter_tol=reverse_stage_adjoint_iter_tol,
                reverse_stage_adjoint_woodbury_rank=reverse_stage_adjoint_woodbury_rank,
                max_reverse_accepted_steps=max_reverse_accepted_steps,
                progress_label=progress_label,
                raw_block_solve=raw_solve,
            )
        )
        benchmark_options = dict(opts)
        benchmark_options.update(use_runtime_payload=True, initial_er_root_ad="off")
        transport_table = benchmark_builder(
            tuple(objective_names), parameter_set, benchmark_options
        )

        setup = _transport_reverse.prepare_reverse_static_setup(
            profiles,
            config=table_context.config,
            runtime=fixed_runtime,
            baseline_state=rooted_state,
            profile_cfg=table_context.profile_cfg,
            initial_er_root_ad="off",
            accepted_step_limit_override=opts.get(
                "accepted_step_limit", accepted_step_limit
            ),
            reverse_segment_length=opts.get(
                "reverse_segment_length", reverse_segment_length
            ),
            ...
            reverse_rhs_transpose_mode=str(
                opts.get("reverse_rhs_transpose_mode", reverse_rhs_transpose_mode)
            ),
            reverse_rhs_pullback_mode=str(opts.get("reverse_rhs_pullback_mode", "separate")),
            reverse_database_initial_support_mode=str(
                opts.get("reverse_database_initial_support_mode", "split")
            ),
            reverse_database_support_preparation_mode=str(
                opts.get("reverse_database_support_preparation_mode", "shared")
            ),
            reverse_database_center_geometry_mode=str(
                opts.get("reverse_database_center_geometry_mode", "scalar_jvp")
            ),
            reverse_database_stage_jacobian_mode=str(
                opts.get("reverse_database_stage_jacobian_mode", "independent")
            ),
            reverse_database_support_objective_mode=str(
                opts.get("reverse_database_support_objective_mode", "scalar")
            ),
            reverse_database_segment_support_mode=str(
                opts.get("reverse_database_segment_support_mode", "inline")
            ),
            ...
            reverse_segment_start_replay_mode=str(
                opts.get("reverse_segment_start_replay_mode", "legacy")
            ),
            reverse_segment_primal_record_mode=str(
                opts.get("reverse_segment_primal_record_mode", "reconstruct")
            ),
            reverse_stage_cotangent_mode=str(
                opts.get("reverse_stage_cotangent_mode", reverse_stage_cotangent_mode)
            ),
            reverse_step_bwd_mode=str(
                opts.get("reverse_step_bwd_mode", reverse_step_bwd_mode)
            ),
            reverse_stage_adjoint_memory_mode=str(
                opts.get(
            ...
            reverse_schedule_artifact_mode=str(
                opts.get("reverse_schedule_artifact_mode", "legacy")
            ),
            max_reverse_accepted_steps=opts.get(
                "max_reverse_accepted_steps", max_reverse_accepted_steps
            ),
        )
        er_bars = _database_full_transport_initial_er_bars(
            rooted_state, runtime=fixed_runtime, reverse_setup=setup
        )

        # The stage's direct-cotangent JIT evaluates objectives at the initial
        # root and therefore is not applicable to final-time transport
        # objectives. The accepted root boundaries used here are the
        # un-jitted selected-root primal and the jitted implicit pullback.

        def _batched_zero(tree):
            zero = _float_delta_tree_like(tree)
            return jax.tree_util.tree_map(
                lambda leaf: jnp.broadcast_to(
                    jnp.asarray(leaf)[None, ...],
                    (len(labels),) + jnp.asarray(leaf).shape,
                ),
                zero,
            )

        _, root_support_bars = root_pullback(
            pre_root_state,
            er_profile,
            finite_mask,
            er_bars,
            _batched_zero(support["geometry"]),
            _batched_zero(support["database"]),
            profiles,
            geometry_leaves,
            database_leaves,
        )
        rows = tuple(labels.index(str(name)) for name in objective_names)
        selected_root_bars = tuple(root_support_bars[row] for row in rows)
        selected_root_bars = fold_recorded_ntx_scan_database_bar_groups_into_support(
            scan_owner, (selected_root_bars,)
        )[0]
        root_assembly = realtime_geometry_transport_reverse_table_from_payload_cotangents(
            objective_labels=tuple(objective_names),
            profile_parameter_labels=(),
            geometry_parameter_labels=transport_table.geometry_parameter_labels,
            objective_values=transport_table.objective_values,
            profile_gradient_matrix=jnp.zeros(
                (len(rows), 0), dtype=transport_table.objective_values.dtype
            ),
            geometry_context=geometry_context,
            baseline_geometry_deltas=baseline_geometry_deltas,
            ...
            support_bars=selected_root_bars,
            support_component_bars_by_name={},
            include_component_pullbacks=False,
            combined_geometry_payload=True,
            payload_kind="ntx_scan_runtime",
            scan_rho=table_context.neoclassical_cfg.get("ntx_scan_rho"),
            scan_surface_backend=str(
                table_context.neoclassical_cfg.get("ntx_scan_surface_backend", "vmec")
            ),
            n_r=int(opts.get("n_r", n_r)),
        ...
        return dataclasses.replace(
            ...
            geometry_gradient_matrix=(
                transport_table.geometry_gradient_matrix
                + root_assembly.table_result.geometry_gradient_matrix
            ),
        )
...
def _database_geometry_active_initial_er_root_only_reverse_table(
    *,
    ...
    # Build the live scan/database from the current VMEC state before splitting
    # its recorded scan owner. Previously this always split the baseline
    # runtime, so transport objective values became stale after an optimizer
...
def geometry_full_ad_objective_cotangent_table(
    *,
    ...
    values_by_name, vmec_state_bars = geometry_full_ad_objective_table_pullback_from_param_vector(
        ...
        requested_objective_names=canonical_objectives,
        lane=lane,
        max_iter=max_iter,
        step_size=step_size,
        final_vmec_pullback_mode="raw_block_transpose",
