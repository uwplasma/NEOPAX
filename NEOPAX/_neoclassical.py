...
from ._interpolators import get_Dij_generic, get_Dij_ntss
...
def _database_uses_ntss(database) -> bool:
    return bool(int(np.asarray(database.interp_mode))) if hasattr(database, "interp_mode") else False


def _make_get_Lij_matrix(dij_fn):
    @jit
    def _get_Lij_matrix(species, energy_grid, geometry, database, index_species, r_index, Er, temperature, density, v_thermal):
        Lij = jnp.zeros((3, 3))
        ...
        Dij = jax.vmap(dij_fn, in_axes=(None, 0, 0, None))(geometry.r_grid[r_index], nu_vnew_a, Er_vnew_a, database)
    ...
    return _get_Lij_matrix


def _make_get_Lij_matrix_local(dij_fn):
    @jit
    def _get_Lij_matrix_local(species, energy_grid, geometry, database, index_species, r_index, Er_value, temperature, density, v_thermal):
        Lij = jnp.zeros((3, 3))
        vth_a = v_thermal[index_species, r_index]
        v_new_a = energy_grid.v_norm * vth_a
    ...
    return _get_Lij_matrix_local


def _make_get_Lij_matrix_at_radius(dij_fn):
    @jit
    def _get_Lij_matrix_at_radius(species, energy_grid, geometry, database, index_species, radius_value, Er_value, temperature_local, density_local, v_thermal_local):
        Lij = jnp.zeros((3, 3))
        ...
        L13_fac_a = -1.0 / jnp.sqrt(jnp.pi) * (species.mass[index_species] / species.charge[index_species]) * vth_a**2
        L33_fac_a = -1.0 / jnp.sqrt(jnp.pi) * vth_a
        Dij = jax.vmap(dij_fn, in_axes=(None, 0, 0, None))(radius_value, nu_vnew_a, Er_vnew_a, database)
        D11_a = -(10**Dij.at[:, 0].get())
        D13_a = -Dij.at[:, 1].get()
        D33_a = -jnp.true_divide(Dij.at[:, 2].get(), nu_vnew_a)
        Lij = Lij.at[0, 0].set(L11_fac_a * jnp.sum(energy_grid.L11_weight * energy_grid.xWeights * D11_a))
    ...
    return _get_Lij_matrix_at_radius


_get_Lij_matrix_generic = _make_get_Lij_matrix(get_Dij_generic)
_get_Lij_matrix_ntss = _make_get_Lij_matrix(get_Dij_ntss)
_get_Lij_matrix_local_generic = _make_get_Lij_matrix_local(get_Dij_generic)
_get_Lij_matrix_local_ntss = _make_get_Lij_matrix_local(get_Dij_ntss)
_get_Lij_matrix_at_radius_generic = _make_get_Lij_matrix_at_radius(get_Dij_generic)
_get_Lij_matrix_at_radius_ntss = _make_get_Lij_matrix_at_radius(get_Dij_ntss)


def get_Lij_matrix(species, energy_grid, geometry, database, index_species, r_index, Er, temperature, density, v_thermal):
    fn = _get_Lij_matrix_ntss if _database_uses_ntss(database) else _get_Lij_matrix_generic
    return fn(species, energy_grid, geometry, database, index_species, r_index, Er, temperature, density, v_thermal)
...
def get_Lij_matrix_local(species, energy_grid, geometry, database, index_species, r_index, Er_value, temperature, density, v_thermal):
    fn = _get_Lij_matrix_local_ntss if _database_uses_ntss(database) else _get_Lij_matrix_local_generic
    return fn(species, energy_grid, geometry, database, index_species, r_index, Er_value, temperature, density, v_thermal)
...
def get_Lij_matrix_at_radius(species, energy_grid, geometry, database, index_species, radius_value, Er_value, temperature_local, density_local, v_thermal_local):
    fn = _get_Lij_matrix_at_radius_ntss if _database_uses_ntss(database) else _get_Lij_matrix_at_radius_generic
    return fn(species, energy_grid, geometry, database, index_species, radius_value, Er_value, temperature_local, density_local, v_thermal_local)


def _make_get_Neoclassical_Fluxes(get_lij_matrix_fn):
    @jit
    def _get_Neoclassical_Fluxes(
    species,
    energy_grid,
    geometry,
    database,
    ...
    temperature_right_grad_constraint=None,
        ...
        Lij = jax.vmap(
            lambda a: jax.vmap(
                lambda r: get_lij_matrix_fn(species, energy_grid, geometry, database, a, r, Er, temperature, density, v_thermal),
                in_axes=(0),
    ...
    return _get_Neoclassical_Fluxes


def _make_get_Neoclassical_Fluxes_Faces(get_lij_matrix_at_radius_fn):
    @jit
    def _get_Neoclassical_Fluxes_Faces(
    species,
    energy_grid,
    geometry,
    database,
    ...
    dTdr_faces,
        ...
        Lij_faces = jax.vmap(
            lambda a: jax.vmap(
                lambda radius_value, er_value, temperature_local, density_local, vthermal_local: get_lij_matrix_at_radius_fn(
                    species,
                    energy_grid,
                    geometry,
                    database,
    ...
    return _get_Neoclassical_Fluxes_Faces


_get_Neoclassical_Fluxes_generic = _make_get_Neoclassical_Fluxes(_get_Lij_matrix_generic)
_get_Neoclassical_Fluxes_ntss = _make_get_Neoclassical_Fluxes(_get_Lij_matrix_ntss)
_get_Neoclassical_Fluxes_Faces_generic = _make_get_Neoclassical_Fluxes_Faces(_get_Lij_matrix_at_radius_generic)
_get_Neoclassical_Fluxes_Faces_ntss = _make_get_Neoclassical_Fluxes_Faces(_get_Lij_matrix_at_radius_ntss)
...
def get_Neoclassical_Fluxes(
    ...
    fn = _get_Neoclassical_Fluxes_ntss if _database_uses_ntss(database) else _get_Neoclassical_Fluxes_generic
    return fn(
        species,
        energy_grid,
        geometry,
...
def get_Neoclassical_Fluxes_Faces(
    ...
    fn = _get_Neoclassical_Fluxes_Faces_ntss if _database_uses_ntss(database) else _get_Neoclassical_Fluxes_Faces_generic
    return fn(
        species,
        energy_grid,
        geometry,
