...
from ._interpolators_ntss_preprocessed import (
    get_Dij_ntss_preprocessed,
    get_Dij_ntss_preprocessed_fast,
)
from ._interpolators_preprocessed import (
    ...
    get_Dij_preprocessed_3d_ntss_radius_ntss1d_generic,
)
...
def _uses_preprocessed_3d_ntss_radius_ntss1d(database):
    return isinstance(database, PreprocessedMonoenergetic3DNTSSRadiusNTSS1D)


def _uses_preprocessed_3d_ntss_radius_ntss1d_fixednu(database):
    return isinstance(database, PreprocessedMonoenergetic3DNTSSRadiusNTSS1DFixedNU)


def _uses_preprocessed_3d_ntss_radius_ntss1d_generic(database):
    return isinstance(database, PreprocessedMonoenergetic3DNTSSRadiusNTSS1DGeneric)
...
def _uses_ntss_preprocessed_fast(database):
    return isinstance(database, NTSSPreprocessedFastMonoenergetic)
...
def _get_Lij_matrix_ntss_preprocessed_fast(species, energy_grid, geometry, database, index_species, r_index, Er, temperature, density, v_thermal, collisionality_kind=COLLISIONALITY_MODEL_DEFAULT):
    Lij = jnp.zeros((3, 3))
    vth_a = v_thermal[index_species, r_index]
    v_new_a = energy_grid.v_norm * vth_a
    Er_vnew_a = Er[r_index] * 1.0e3 / v_new_a
    ...
    L13_fac_a = -1.0 / jnp.sqrt(jnp.pi) * (species.mass[index_species] / species.charge[index_species]) * vth_a**2
    L33_fac_a = -1.0 / jnp.sqrt(jnp.pi) * vth_a
    Dij = jax.vmap(get_Dij_ntss_preprocessed_fast, in_axes=(None, 0, 0, None))(geometry.r_grid[r_index], nu_vnew_a, Er_vnew_a, database)
    D11_a = -(10**Dij.at[:, 0].get())
    D13_a = -Dij.at[:, 1].get()
    D33_a = -jnp.true_divide(Dij.at[:, 2].get(), nu_vnew_a)
    Lij = Lij.at[0, 0].set(L11_fac_a * jnp.sum(energy_grid.L11_weight * energy_grid.xWeights * D11_a))
...
@jit
def _get_Lij_matrix_preprocessed_3d_ntss_radius_ntss1d(species, energy_grid, geometry, database, index_species, r_index, Er, temperature, density, v_thermal, collisionality_kind=COLLISIONALITY_MODEL_DEFAULT):
    Lij = jnp.zeros((3, 3))
    vth_a = v_thermal[index_species, r_index]
    v_new_a = energy_grid.v_norm * vth_a
    Er_vnew_a = Er[r_index] * 1.0e3 / v_new_a
    ...
    L13_fac_a = -1.0 / jnp.sqrt(jnp.pi) * (species.mass[index_species] / species.charge[index_species]) * vth_a**2
    L33_fac_a = -1.0 / jnp.sqrt(jnp.pi) * vth_a
    Dij = jax.vmap(get_Dij_preprocessed_3d_ntss_radius_ntss1d, in_axes=(None, 0, 0, None))(geometry.r_grid[r_index], nu_vnew_a, Er_vnew_a, database)
    D11_a = -(10**Dij.at[:, 0].get())
    D13_a = -Dij.at[:, 1].get()
    D33_a = -jnp.true_divide(Dij.at[:, 2].get(), nu_vnew_a)
    Lij = Lij.at[0, 0].set(L11_fac_a * jnp.sum(energy_grid.L11_weight * energy_grid.xWeights * D11_a))
...
@jit
def _get_Lij_matrix_preprocessed_3d_ntss_radius_ntss1d_fixednu(species, energy_grid, geometry, database, index_species, r_index, Er, temperature, density, v_thermal, collisionality_kind=COLLISIONALITY_MODEL_DEFAULT):
    Lij = jnp.zeros((3, 3))
    vth_a = v_thermal[index_species, r_index]
    v_new_a = energy_grid.v_norm * vth_a
    Er_vnew_a = Er[r_index] * 1.0e3 / v_new_a
    ...
    L13_fac_a = -1.0 / jnp.sqrt(jnp.pi) * (species.mass[index_species] / species.charge[index_species]) * vth_a**2
    L33_fac_a = -1.0 / jnp.sqrt(jnp.pi) * vth_a
    Dij = jax.vmap(get_Dij_preprocessed_3d_ntss_radius_ntss1d_fixednu, in_axes=(None, 0, 0, None))(geometry.r_grid[r_index], nu_vnew_a, Er_vnew_a, database)
    D11_a = -(10**Dij.at[:, 0].get())
    D13_a = -Dij.at[:, 1].get()
    D33_a = -jnp.true_divide(Dij.at[:, 2].get(), nu_vnew_a)
    Lij = Lij.at[0, 0].set(L11_fac_a * jnp.sum(energy_grid.L11_weight * energy_grid.xWeights * D11_a))
...
@jit
def _get_Lij_matrix_preprocessed_3d_ntss_radius_ntss1d_generic(species, energy_grid, geometry, database, index_species, r_index, Er, temperature, density, v_thermal, collisionality_kind=COLLISIONALITY_MODEL_DEFAULT):
    Lij = jnp.zeros((3, 3))
    vth_a = v_thermal[index_species, r_index]
    v_new_a = energy_grid.v_norm * vth_a
    Er_vnew_a = Er[r_index] * 1.0e3 / v_new_a
    ...
    L13_fac_a = -1.0 / jnp.sqrt(jnp.pi) * (species.mass[index_species] / species.charge[index_species]) * vth_a**2
    L33_fac_a = -1.0 / jnp.sqrt(jnp.pi) * vth_a
    Dij = jax.vmap(get_Dij_preprocessed_3d_ntss_radius_ntss1d_generic, in_axes=(None, 0, 0, None))(geometry.r_grid[r_index], nu_vnew_a, Er_vnew_a, database)
    D11_a = -(10**Dij.at[:, 0].get())
    D13_a = -Dij.at[:, 1].get()
    D33_a = -jnp.true_divide(Dij.at[:, 2].get(), nu_vnew_a)
    Lij = Lij.at[0, 0].set(L11_fac_a * jnp.sum(energy_grid.L11_weight * energy_grid.xWeights * D11_a))
...
@jit
def _get_Lij_matrix_at_radius_preprocessed_3d_ntss_radius_ntss1d(species, energy_grid, geometry, database, index_species, radius_value, Er_value, temperature_local, density_local, v_thermal_local, collisionality_kind=COLLISIONALITY_MODEL_DEFAULT):
    Lij = jnp.zeros((3, 3))
    vth_a = v_thermal_local[index_species]
    v_new_a = energy_grid.v_norm * vth_a
    Er_vnew_a = Er_value * 1.0e3 / v_new_a
    ...
    L13_fac_a = -1.0 / jnp.sqrt(jnp.pi) * (species.mass[index_species] / species.charge[index_species]) * vth_a**2
    L33_fac_a = -1.0 / jnp.sqrt(jnp.pi) * vth_a
    Dij = jax.vmap(get_Dij_preprocessed_3d_ntss_radius_ntss1d, in_axes=(None, 0, 0, None))(radius_value, nu_vnew_a, Er_vnew_a, database)
    D11_a = -(10**Dij.at[:, 0].get())
    D13_a = -Dij.at[:, 1].get()
    D33_a = -jnp.true_divide(Dij.at[:, 2].get(), nu_vnew_a)
    Lij = Lij.at[0, 0].set(L11_fac_a * jnp.sum(energy_grid.L11_weight * energy_grid.xWeights * D11_a))
...
@jit
def _get_Lij_matrix_at_radius_preprocessed_3d_ntss_radius_ntss1d_fixednu(species, energy_grid, geometry, database, index_species, radius_value, Er_value, temperature_local, density_local, v_thermal_local, collisionality_kind=COLLISIONALITY_MODEL_DEFAULT):
    Lij = jnp.zeros((3, 3))
    vth_a = v_thermal_local[index_species]
    v_new_a = energy_grid.v_norm * vth_a
    Er_vnew_a = Er_value * 1.0e3 / v_new_a
    ...
    L13_fac_a = -1.0 / jnp.sqrt(jnp.pi) * (species.mass[index_species] / species.charge[index_species]) * vth_a**2
    L33_fac_a = -1.0 / jnp.sqrt(jnp.pi) * vth_a
    Dij = jax.vmap(get_Dij_preprocessed_3d_ntss_radius_ntss1d_fixednu, in_axes=(None, 0, 0, None))(radius_value, nu_vnew_a, Er_vnew_a, database)
    D11_a = -(10**Dij.at[:, 0].get())
    D13_a = -Dij.at[:, 1].get()
    D33_a = -jnp.true_divide(Dij.at[:, 2].get(), nu_vnew_a)
    Lij = Lij.at[0, 0].set(L11_fac_a * jnp.sum(energy_grid.L11_weight * energy_grid.xWeights * D11_a))
...
@jit
def _get_Lij_matrix_at_radius_preprocessed_3d_ntss_radius_ntss1d_generic(species, energy_grid, geometry, database, index_species, radius_value, Er_value, temperature_local, density_local, v_thermal_local, collisionality_kind=COLLISIONALITY_MODEL_DEFAULT):
    Lij = jnp.zeros((3, 3))
    vth_a = v_thermal_local[index_species]
    v_new_a = energy_grid.v_norm * vth_a
    Er_vnew_a = Er_value * 1.0e3 / v_new_a
    ...
    L13_fac_a = -1.0 / jnp.sqrt(jnp.pi) * (species.mass[index_species] / species.charge[index_species]) * vth_a**2
    L33_fac_a = -1.0 / jnp.sqrt(jnp.pi) * vth_a
    Dij = jax.vmap(get_Dij_preprocessed_3d_ntss_radius_ntss1d_generic, in_axes=(None, 0, 0, None))(radius_value, nu_vnew_a, Er_vnew_a, database)
    D11_a = -(10**Dij.at[:, 0].get())
    D13_a = -Dij.at[:, 1].get()
    D33_a = -jnp.true_divide(Dij.at[:, 2].get(), nu_vnew_a)
    Lij = Lij.at[0, 0].set(L11_fac_a * jnp.sum(energy_grid.L11_weight * energy_grid.xWeights * D11_a))
...
def _get_Lij_matrix_at_radius_ntss_preprocessed_fast(species, energy_grid, geometry, database, index_species, radius_value, Er_value, temperature_local, density_local, v_thermal_local, collisionality_kind=COLLISIONALITY_MODEL_DEFAULT):
    Lij = jnp.zeros((3, 3))
    vth_a = v_thermal_local[index_species]
    v_new_a = energy_grid.v_norm * vth_a
    Er_vnew_a = Er_value * 1.0e3 / v_new_a
    ...
    L13_fac_a = -1.0 / jnp.sqrt(jnp.pi) * (species.mass[index_species] / species.charge[index_species]) * vth_a**2
    L33_fac_a = -1.0 / jnp.sqrt(jnp.pi) * vth_a
    Dij = jax.vmap(get_Dij_ntss_preprocessed_fast, in_axes=(None, 0, 0, None))(radius_value, nu_vnew_a, Er_vnew_a, database)
    D11_a = -(10**Dij.at[:, 0].get())
    D13_a = -Dij.at[:, 1].get()
    D33_a = -jnp.true_divide(Dij.at[:, 2].get(), nu_vnew_a)
    Lij = Lij.at[0, 0].set(L11_fac_a * jnp.sum(energy_grid.L11_weight * energy_grid.xWeights * D11_a))
...
@jit
def _get_Neoclassical_Fluxes_ntss_preprocessed_fast(
    species,
    energy_grid,
    geometry,
    database,
    ...
    Lij = jax.vmap(
        lambda a: jax.vmap(
            lambda r: _get_Lij_matrix_ntss_preprocessed_fast(species, energy_grid, geometry, database, a, r, Er, temperature, density, v_thermal, collisionality_kind),
            in_axes=(0),
        )(geometry.full_grid_indices),
        in_axes=(0),
    )(species.species_indices)
...
@jit
def _get_Neoclassical_Fluxes_preprocessed_3d_ntss_radius_ntss1d(
    species,
    energy_grid,
    geometry,
    database,
    ...
    Lij = jax.vmap(
        lambda a: jax.vmap(
            lambda r: _get_Lij_matrix_preprocessed_3d_ntss_radius_ntss1d(species, energy_grid, geometry, database, a, r, Er, temperature, density, v_thermal, collisionality_kind),
            in_axes=(0),
        )(geometry.full_grid_indices),
        in_axes=(0),
    )(species.species_indices)
...
@jit
def _get_Neoclassical_Fluxes_preprocessed_3d_ntss_radius_ntss1d_fixednu(
    species,
    energy_grid,
    geometry,
    database,
    ...
    Lij = jax.vmap(
        lambda a: jax.vmap(
            lambda r: _get_Lij_matrix_preprocessed_3d_ntss_radius_ntss1d_fixednu(species, energy_grid, geometry, database, a, r, Er, temperature, density, v_thermal, collisionality_kind),
            in_axes=(0),
        )(geometry.full_grid_indices),
        in_axes=(0),
    )(species.species_indices)
...
@jit
def _get_Neoclassical_Fluxes_preprocessed_3d_ntss_radius_ntss1d_generic(
    species,
    energy_grid,
    geometry,
    database,
    ...
    Lij = jax.vmap(
        lambda a: jax.vmap(
            lambda r: _get_Lij_matrix_preprocessed_3d_ntss_radius_ntss1d_generic(species, energy_grid, geometry, database, a, r, Er, temperature, density, v_thermal, collisionality_kind),
            in_axes=(0),
        )(geometry.full_grid_indices),
        in_axes=(0),
    )(species.species_indices)
...
@jit
def _get_Neoclassical_Fluxes_Faces_ntss_preprocessed_fast(
    species,
    energy_grid,
    geometry,
    database,
    ...
    Lij_faces = jax.vmap(
        lambda a: jax.vmap(
            lambda radius_value, er_value, temperature_local, density_local, vthermal_local: _get_Lij_matrix_at_radius_ntss_preprocessed_fast(
                species,
                energy_grid,
                geometry,
                database,
...
@jit
def _get_Neoclassical_Fluxes_Faces_preprocessed_3d_ntss_radius_ntss1d(
    species,
    energy_grid,
    geometry,
    database,
    ...
    Lij_faces = jax.vmap(
        lambda a: jax.vmap(
            lambda radius_value, er_value, temperature_local, density_local, vthermal_local: _get_Lij_matrix_at_radius_preprocessed_3d_ntss_radius_ntss1d(
                species,
                energy_grid,
                geometry,
                database,
...
@jit
def _get_Neoclassical_Fluxes_Faces_preprocessed_3d_ntss_radius_ntss1d_fixednu(
    species,
    energy_grid,
    geometry,
    database,
    ...
    Lij_faces = jax.vmap(
        lambda a: jax.vmap(
            lambda radius_value, er_value, temperature_local, density_local, vthermal_local: _get_Lij_matrix_at_radius_preprocessed_3d_ntss_radius_ntss1d_fixednu(
                species,
                energy_grid,
                geometry,
                database,
...
@jit
def _get_Neoclassical_Fluxes_Faces_preprocessed_3d_ntss_radius_ntss1d_generic(
    species,
    energy_grid,
    geometry,
    database,
    ...
    Lij_faces = jax.vmap(
        lambda a: jax.vmap(
            lambda radius_value, er_value, temperature_local, density_local, vthermal_local: _get_Lij_matrix_at_radius_preprocessed_3d_ntss_radius_ntss1d_generic(
                species,
                energy_grid,
                geometry,
                database,
...
def get_Neoclassical_Fluxes(
    ...
    if _uses_ntss_preprocessed_fast(database):
        return _get_Neoclassical_Fluxes_ntss_preprocessed_fast(
            species,
            energy_grid,
            geometry,
            database,
    ...
    if _uses_preprocessed_3d_ntss_radius_ntss1d_fixednu(database):
        return _get_Neoclassical_Fluxes_preprocessed_3d_ntss_radius_ntss1d_fixednu(
            species,
            energy_grid,
            geometry,
            database,
    ...
    if _uses_preprocessed_3d_ntss_radius_ntss1d_generic(database):
        return _get_Neoclassical_Fluxes_preprocessed_3d_ntss_radius_ntss1d_generic(
            species,
            energy_grid,
            geometry,
            database,
    ...
    if _uses_preprocessed_3d_ntss_radius_ntss1d(database):
        return _get_Neoclassical_Fluxes_preprocessed_3d_ntss_radius_ntss1d(
            species,
            energy_grid,
            geometry,
            database,
...
def get_Neoclassical_Fluxes_Faces(
    ...
    collisionality_kind = _collisionality_kind(collisionality_model)
    if _uses_ntss_preprocessed_fast(database):
        return _get_Neoclassical_Fluxes_Faces_ntss_preprocessed_fast(
            species,
            energy_grid,
            geometry,
            database,
    ...
    if _uses_preprocessed_3d_ntss_radius_ntss1d_fixednu(database):
        return _get_Neoclassical_Fluxes_Faces_preprocessed_3d_ntss_radius_ntss1d_fixednu(
            species,
            energy_grid,
            geometry,
            database,
    ...
    if _uses_preprocessed_3d_ntss_radius_ntss1d_generic(database):
        return _get_Neoclassical_Fluxes_Faces_preprocessed_3d_ntss_radius_ntss1d_generic(
            species,
            energy_grid,
            geometry,
            database,
    ...
    if _uses_preprocessed_3d_ntss_radius_ntss1d(database):
        return _get_Neoclassical_Fluxes_Faces_preprocessed_3d_ntss_radius_ntss1d(
            species,
            energy_grid,
            geometry,
            database,
