...
from ._interpolators_preprocessed import (
    ...
    get_Dij_preprocessed_3d_er_raw,
    get_Dij_preprocessed_3d_ertilde,
    get_Dij_preprocessed_3d_monotonic,
...
def _uses_preprocessed_3d_er_raw(database):
    return isinstance(database, PreprocessedMonoenergetic3DErRaw)


def _uses_preprocessed_3d_ertilde(database):
    return isinstance(database, PreprocessedMonoenergetic3DErTilde)


def _uses_preprocessed_3d_monotonic(database):
    return isinstance(database, PreprocessedMonoenergetic3DMonotonic)
...
@jit
def _get_Lij_matrix_preprocessed_er_raw(species, energy_grid, geometry, database, index_species, r_index, Er, temperature, density, v_thermal, collisionality_kind=COLLISIONALITY_MODEL_DEFAULT):
    Lij = jnp.zeros((3, 3))
    vth_a = v_thermal[index_species, r_index]
    v_new_a = energy_grid.v_norm * vth_a
    Er_vnew_a = Er[r_index] * 1.0e3 / v_new_a
    ...
    L13_fac_a = -1.0 / jnp.sqrt(jnp.pi) * (species.mass[index_species] / species.charge[index_species]) * vth_a**2
    L33_fac_a = -1.0 / jnp.sqrt(jnp.pi) * vth_a
    Dij = jax.vmap(get_Dij_preprocessed_3d_er_raw, in_axes=(None, 0, 0, None))(geometry.r_grid[r_index], nu_vnew_a, Er_vnew_a, database)
    D11_a = -(10**Dij.at[:, 0].get())
    D13_a = -Dij.at[:, 1].get()
    D33_a = -jnp.true_divide(Dij.at[:, 2].get(), nu_vnew_a)
    Lij = Lij.at[0, 0].set(L11_fac_a * jnp.sum(energy_grid.L11_weight * energy_grid.xWeights * D11_a))
...
@jit
def _get_Lij_matrix_at_radius_preprocessed_er_raw(species, energy_grid, geometry, database, index_species, radius_value, Er_value, temperature_local, density_local, v_thermal_local, collisionality_kind=COLLISIONALITY_MODEL_DEFAULT):
    Lij = jnp.zeros((3, 3))
    vth_a = v_thermal_local[index_species]
    v_new_a = energy_grid.v_norm * vth_a
    Er_vnew_a = Er_value * 1.0e3 / v_new_a
    ...
    L13_fac_a = -1.0 / jnp.sqrt(jnp.pi) * (species.mass[index_species] / species.charge[index_species]) * vth_a**2
    L33_fac_a = -1.0 / jnp.sqrt(jnp.pi) * vth_a
    Dij = jax.vmap(get_Dij_preprocessed_3d_er_raw, in_axes=(None, 0, 0, None))(radius_value, nu_vnew_a, Er_vnew_a, database)
    D11_a = -(10**Dij.at[:, 0].get())
    D13_a = -Dij.at[:, 1].get()
    D33_a = -jnp.true_divide(Dij.at[:, 2].get(), nu_vnew_a)
    Lij = Lij.at[0, 0].set(L11_fac_a * jnp.sum(energy_grid.L11_weight * energy_grid.xWeights * D11_a))
...
@jit
def _get_Lij_matrix_preprocessed_ertilde(species, energy_grid, geometry, database, index_species, r_index, Er, temperature, density, v_thermal, collisionality_kind=COLLISIONALITY_MODEL_DEFAULT):
    Lij = jnp.zeros((3, 3))
    vth_a = v_thermal[index_species, r_index]
    v_new_a = energy_grid.v_norm * vth_a
    Er_vnew_a = Er[r_index] * 1.0e3 / v_new_a
    ...
    L13_fac_a = -1.0 / jnp.sqrt(jnp.pi) * (species.mass[index_species] / species.charge[index_species]) * vth_a**2
    L33_fac_a = -1.0 / jnp.sqrt(jnp.pi) * vth_a
    Dij = jax.vmap(get_Dij_preprocessed_3d_ertilde, in_axes=(None, 0, 0, None))(geometry.r_grid[r_index], nu_vnew_a, Er_vnew_a, database)
    D11_a = -(10**Dij.at[:, 0].get())
    D13_a = -Dij.at[:, 1].get()
    D33_a = -jnp.true_divide(Dij.at[:, 2].get(), nu_vnew_a)
    Lij = Lij.at[0, 0].set(L11_fac_a * jnp.sum(energy_grid.L11_weight * energy_grid.xWeights * D11_a))
...
@jit
def _get_Lij_matrix_at_radius_preprocessed_ertilde(species, energy_grid, geometry, database, index_species, radius_value, Er_value, temperature_local, density_local, v_thermal_local, collisionality_kind=COLLISIONALITY_MODEL_DEFAULT):
    Lij = jnp.zeros((3, 3))
    vth_a = v_thermal_local[index_species]
    v_new_a = energy_grid.v_norm * vth_a
    Er_vnew_a = Er_value * 1.0e3 / v_new_a
    ...
    L13_fac_a = -1.0 / jnp.sqrt(jnp.pi) * (species.mass[index_species] / species.charge[index_species]) * vth_a**2
    L33_fac_a = -1.0 / jnp.sqrt(jnp.pi) * vth_a
    Dij = jax.vmap(get_Dij_preprocessed_3d_ertilde, in_axes=(None, 0, 0, None))(radius_value, nu_vnew_a, Er_vnew_a, database)
    D11_a = -(10**Dij.at[:, 0].get())
    D13_a = -Dij.at[:, 1].get()
    D33_a = -jnp.true_divide(Dij.at[:, 2].get(), nu_vnew_a)
    Lij = Lij.at[0, 0].set(L11_fac_a * jnp.sum(energy_grid.L11_weight * energy_grid.xWeights * D11_a))
...
@jit
def _get_Lij_matrix_preprocessed_monotonic(species, energy_grid, geometry, database, index_species, r_index, Er, temperature, density, v_thermal, collisionality_kind=COLLISIONALITY_MODEL_DEFAULT):
    Lij = jnp.zeros((3, 3))
    vth_a = v_thermal[index_species, r_index]
    v_new_a = energy_grid.v_norm * vth_a
    Er_vnew_a = Er[r_index] * 1.0e3 / v_new_a
    ...
    L13_fac_a = -1.0 / jnp.sqrt(jnp.pi) * (species.mass[index_species] / species.charge[index_species]) * vth_a**2
    L33_fac_a = -1.0 / jnp.sqrt(jnp.pi) * vth_a
    Dij = jax.vmap(get_Dij_preprocessed_3d_monotonic, in_axes=(None, 0, 0, None))(geometry.r_grid[r_index], nu_vnew_a, Er_vnew_a, database)
    D11_a = -(10**Dij.at[:, 0].get())
    D13_a = -Dij.at[:, 1].get()
    D33_a = -jnp.true_divide(Dij.at[:, 2].get(), nu_vnew_a)
    Lij = Lij.at[0, 0].set(L11_fac_a * jnp.sum(energy_grid.L11_weight * energy_grid.xWeights * D11_a))
...
@jit
def _get_Lij_matrix_at_radius_preprocessed_monotonic(species, energy_grid, geometry, database, index_species, radius_value, Er_value, temperature_local, density_local, v_thermal_local, collisionality_kind=COLLISIONALITY_MODEL_DEFAULT):
    Lij = jnp.zeros((3, 3))
    vth_a = v_thermal_local[index_species]
    v_new_a = energy_grid.v_norm * vth_a
    Er_vnew_a = Er_value * 1.0e3 / v_new_a
    ...
    L13_fac_a = -1.0 / jnp.sqrt(jnp.pi) * (species.mass[index_species] / species.charge[index_species]) * vth_a**2
    L33_fac_a = -1.0 / jnp.sqrt(jnp.pi) * vth_a
    Dij = jax.vmap(get_Dij_preprocessed_3d_monotonic, in_axes=(None, 0, 0, None))(radius_value, nu_vnew_a, Er_vnew_a, database)
    D11_a = -(10**Dij.at[:, 0].get())
    D13_a = -Dij.at[:, 1].get()
    D33_a = -jnp.true_divide(Dij.at[:, 2].get(), nu_vnew_a)
    Lij = Lij.at[0, 0].set(L11_fac_a * jnp.sum(energy_grid.L11_weight * energy_grid.xWeights * D11_a))
...
@jit
def _get_Neoclassical_Fluxes_preprocessed_monotonic(
    species,
    energy_grid,
    geometry,
    database,
    ...
    Lij = jax.vmap(
        lambda a: jax.vmap(
            lambda r: _get_Lij_matrix_preprocessed_monotonic(species, energy_grid, geometry, database, a, r, Er, temperature, density, v_thermal, collisionality_kind),
            in_axes=(0),
        )(geometry.full_grid_indices),
        in_axes=(0),
    )(species.species_indices)
...
@jit
def _get_Neoclassical_Fluxes_preprocessed_er_raw(
    species,
    energy_grid,
    geometry,
    database,
    ...
    Lij = jax.vmap(
        lambda a: jax.vmap(
            lambda r: _get_Lij_matrix_preprocessed_er_raw(species, energy_grid, geometry, database, a, r, Er, temperature, density, v_thermal, collisionality_kind),
            in_axes=(0),
        )(geometry.full_grid_indices),
        in_axes=(0),
    )(species.species_indices)
...
@jit
def _get_Neoclassical_Fluxes_preprocessed_ertilde(
    species,
    energy_grid,
    geometry,
    database,
    ...
    Lij = jax.vmap(
        lambda a: jax.vmap(
            lambda r: _get_Lij_matrix_preprocessed_ertilde(species, energy_grid, geometry, database, a, r, Er, temperature, density, v_thermal, collisionality_kind),
            in_axes=(0),
        )(geometry.full_grid_indices),
        in_axes=(0),
    )(species.species_indices)
...
@jit
def _get_Neoclassical_Fluxes_Faces_preprocessed_ertilde(
    species,
    energy_grid,
    geometry,
    database,
    ...
    Lij_faces = jax.vmap(
        lambda a: jax.vmap(
            lambda radius_value, er_value, temperature_local, density_local, vthermal_local: _get_Lij_matrix_at_radius_preprocessed_ertilde(
                species,
                energy_grid,
                geometry,
                database,
...
@jit
def _get_Neoclassical_Fluxes_Faces_preprocessed_monotonic(
    species,
    energy_grid,
    geometry,
    database,
    ...
    Lij_faces = jax.vmap(
        lambda a: jax.vmap(
            lambda radius_value, er_value, temperature_local, density_local, vthermal_local: _get_Lij_matrix_at_radius_preprocessed_monotonic(
                species,
                energy_grid,
                geometry,
                database,
...
@jit
def _get_Neoclassical_Fluxes_Faces_preprocessed_er_raw(
    species,
    energy_grid,
    geometry,
    database,
    ...
    Lij_faces = jax.vmap(
        lambda a: jax.vmap(
            lambda radius_value, er_value, temperature_local, density_local, vthermal_local: _get_Lij_matrix_at_radius_preprocessed_er_raw(
                species,
                energy_grid,
                geometry,
                database,
...
def get_Neoclassical_Fluxes(
    ...
    if _uses_preprocessed_3d_er_raw(database):
        return _get_Neoclassical_Fluxes_preprocessed_er_raw(
            species,
            energy_grid,
            geometry,
            database,
    ...
    if _uses_preprocessed_3d_ertilde(database):
        return _get_Neoclassical_Fluxes_preprocessed_ertilde(
            species,
            energy_grid,
            geometry,
            database,
    ...
    if _uses_preprocessed_3d_monotonic(database):
        return _get_Neoclassical_Fluxes_preprocessed_monotonic(
            species,
            energy_grid,
            geometry,
            database,
...
def get_Neoclassical_Fluxes_Faces(
    ...
    if _uses_preprocessed_3d_er_raw(database):
        return _get_Neoclassical_Fluxes_Faces_preprocessed_er_raw(
            species,
            energy_grid,
            geometry,
            database,
    ...
    if _uses_preprocessed_3d_ertilde(database):
        return _get_Neoclassical_Fluxes_Faces_preprocessed_ertilde(
            species,
            energy_grid,
            geometry,
            database,
    ...
    if _uses_preprocessed_3d_monotonic(database):
        return _get_Neoclassical_Fluxes_Faces_preprocessed_monotonic(
            species,
            energy_grid,
            geometry,
            database,
