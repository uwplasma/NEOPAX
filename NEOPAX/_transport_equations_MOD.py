...
class TemperatureEquation(EquationBase):
    ...
    source_model: callable = dataclasses.field(repr=False, default=None)
    species: object = dataclasses.field(repr=False, default=None)
    work_source_mode: str = "centered"  # New option: "local" or "centered"
    ...
    def debug_components(self, state, fluxes=None):
        ...
        # --- Er*Gamma work term logic ---
        mode = str(getattr(self, "work_source_mode", "centered")).strip().lower()
        if mode in {"ambipolar_local", "transport_local", "local"}:
            # Use cell-centered Gamma directly
            gamma_for_work = PARTICLE_FLUX_PHYSICAL_TO_STATE * fluxes["Gamma"]
        else:
            # Average face-centered Gamma to centers
            gamma_faces = self.flux_faces_builder(PARTICLE_FLUX_PHYSICAL_TO_STATE * fluxes["Gamma"])
            gamma_for_work = 0.5 * (gamma_faces[:, :-1] + gamma_faces[:, 1:])
        work_rhs = (
            self.charge_qp[:, None]
            * gamma_for_work
            * state.Er[None, :]
...
def build_temperature_equation(
    ...
    Vprime = field.Vprime
    Vprime_half = field.Vprime_half
    def flux_faces_builder(flux, face_reconstruction="centered", state=None, flux_model=None):
        mode = str(face_reconstruction).strip().lower()
        if mode == "face_eval" and state is not None and flux_model is not None:
            # Reconstruct n, T, Er at faces, then evaluate flux_model at faces
            n_face = 0.5 * (state.density[:, :-1] + state.density[:, 1:])
            T_face = 0.5 * (state.temperature[:, :-1] + state.temperature[:, 1:])
            Er_face = 0.5 * (state.Er[:-1] + state.Er[1:])
            # Call the flux model with face arrays (assume it supports direct arrays)
            flux_face = flux_model({
                "density": n_face,
                "temperature": T_face,
                "Er": Er_face
            })
            # Return the correct flux (already at faces)
            return HEAT_FLUX_PHYSICAL_TO_STATE * flux_face["Q"] if "Q" in flux_face else PARTICLE_FLUX_PHYSICAL_TO_STATE * flux_face["Gamma"]
        else:
    ...
    work_source_mode = None
    if solver_cfg := locals().get("solver_cfg", None):
        work_source_mode = solver_cfg.get("work_source_mode", "centered")
    return TemperatureEquation(
        ...
        work_source_mode=work_source_mode or "centered",
    )
