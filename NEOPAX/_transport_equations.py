...
class ComposedEquationSystem:
    ...
    def build_transport_response(self, state, runtime, t=None):
        """Build a frozen transport-response scaffold around a reference state.

        This is the Phase 10 entry point for future transport-response-backed
        solver modes. The current implementation is intentionally lightweight:
        it preserves the existing black-box RHS path while exposing the base
        working state, base shared fluxes, and assembled source components.
        """
        t_value = jnp.asarray(0.0) if t is None else jnp.asarray(t)
        ...
        density_source_components = None
        if density_eq is not None and getattr(density_eq, "source_model", None) is not None:
            density_source_components = assemble_density_source_components(
                density_eq.source_model(working_state),
                working_state,
                self.species,
            )

        pressure_source_components = None
        if temperature_eq is not None and getattr(temperature_eq, "source_model", None) is not None:
            pressure_source_components = assemble_pressure_source_components(
                temperature_eq.source_model(working_state),
                working_state,
                self.species,
            )

        density_components = None
        if density_eq is not None and hasattr(density_eq, "debug_components"):
            density_components = density_eq.debug_components(working_state, fluxes=shared_fluxes)

        temperature_components = None
        if temperature_eq is not None and hasattr(temperature_eq, "debug_components"):
            temperature_components = temperature_eq.debug_components(working_state, fluxes=shared_fluxes)

        er_components = None
        if er_eq is not None and hasattr(er_eq, "debug_components"):
            er_components = er_eq.debug_components(working_state, fluxes=shared_fluxes)

        base_rhs = self(t_value, state, runtime)
        return TransportResponse(
            rhs_mode=str(self.rhs_mode).strip().lower(),
            reference_time=t_value,
            reference_state=state,
            working_state=working_state,
            base_rhs=base_rhs,
            base_fluxes=shared_fluxes,
            density_source_components=density_source_components,
            pressure_source_components=pressure_source_components,
            density_components=density_components,
            temperature_components=temperature_components,
            er_components=er_components,
        )

    def response_vector_field(self, response, t, state, runtime):
        """Evaluate a frozen transport-response approximation at a new state.

        This Phase 10 prototype keeps the existing black-box path unchanged and
        offers a structured frozen-response path for solver experiments:
        density and thermal transport pieces are frozen from the reference
        response, while Er diffusion remains live in the current state and the
        temperature work term can remain live against the frozen particle flux.
        """
        del t, runtime
        import jax.numpy as jnp
        from ._state import TransportState
        ...
        density_rhs = (
            jnp.asarray(response.density_components.get("density_rhs"))
            if response.density_components is not None and "density_rhs" in response.density_components
            else jnp.zeros_like(state.density)
        )
        if response.density_components is not None and "gamma_divergence_raw" in response.density_components:
            density_rhs = jnp.asarray(response.density_components["gamma_divergence_raw"])
            if response.density_source_components is not None:
                density_rhs = density_rhs + sum_source_components(response.density_source_components, working_state.density)

        pressure_rhs = jnp.zeros_like(state.pressure)
        if response.temperature_components is not None and temperature_eq is not None:
            Q_faces = jnp.asarray(
                response.temperature_components.get("Q_faces", jnp.zeros((state.pressure.shape[0], state.pressure.shape[1] + 1), dtype=state.pressure.dtype))
            )
            gamma_neo_faces = jnp.asarray(
                response.temperature_components.get("Gamma_neo_faces", jnp.zeros_like(Q_faces))
            )
            gamma_turb_faces = jnp.asarray(
                response.temperature_components.get("Gamma_turb_faces", jnp.zeros_like(Q_faces))
            )
            gamma_classical_faces = jnp.asarray(
                response.temperature_components.get("Gamma_classical_faces", jnp.zeros_like(Q_faces))
            )
            temperature_ghost = temperature_eq.temperature_ghost_builder(working_state.temperature)
            temperature_left, temperature_right = _temperature_face_states(
                temperature_ghost,
                temperature_eq.convection_reconstruction,
            )

            def _convective_faces(gamma_faces, enabled):
                if not enabled:
                    return jnp.zeros_like(Q_faces)
                temperature_upwind = jnp.where(gamma_faces >= 0.0, temperature_left, temperature_right)
                return temperature_upwind * gamma_faces

            convective_neo_faces = _convective_faces(gamma_neo_faces, getattr(temperature_eq, "include_neo_convection", False))
            convective_turb_faces = _convective_faces(gamma_turb_faces, getattr(temperature_eq, "include_turbulent_convection", False))
            convective_classical_faces = _convective_faces(gamma_classical_faces, getattr(temperature_eq, "include_classical_convection", False))
            total_energy_flux_faces = Q_faces + convective_neo_faces + convective_turb_faces + convective_classical_faces
            thermal_flux_rhs = jax.vmap(
                lambda flux: conservative_update(flux, temperature_eq.dr_cells, temperature_eq.Vprime, temperature_eq.Vprime_half)
            )(total_energy_flux_faces)

            source_rhs = (
                sum_source_components(response.pressure_source_components, working_state.pressure)
                if response.pressure_source_components is not None
                else jnp.zeros_like(state.pressure)
            )
            work_rhs = jnp.zeros_like(state.pressure)
            if (
                getattr(temperature_eq, "include_work_term", False)
                and response.base_fluxes is not None
                and isinstance(response.base_fluxes, dict)
                and "Gamma" in response.base_fluxes
            ):
                work_rhs = (
                    temperature_eq.charge_qp[:, None]
                    * PARTICLE_FLUX_PHYSICAL_TO_STATE
                    * jnp.asarray(response.base_fluxes["Gamma"], dtype=state.pressure.dtype)
                    * working_state.Er[None, :]
                )
            pressure_rhs = (
                (2.0 / 3.0) * (thermal_flux_rhs + source_rhs + work_rhs)
            ) * temperature_eq.active_species_mask[:, None]

        Er_rhs = jnp.zeros_like(state.Er)
        if response.er_components is not None and er_eq is not None:
            frozen_ambi_term = jnp.asarray(
                response.er_components.get(
                    "ambi_term",
                    jnp.zeros_like(state.Er),
                )
            )
            _, er_diffusion = er_eq._er_diffusion(working_state.Er)
            Er_rhs = er_eq.Er_relax * (er_eq.DEr * er_diffusion - frozen_ambi_term)
            if er_eq.boundary_mode == "floating_ambipolar_edge":
                Er_rhs = Er_rhs.at[-1].set(-er_eq.Er_relax * frozen_ambi_term[-1])
...
class TransportResponse:
    ...
    base_rhs: object
    base_fluxes: object | None = None
    density_source_components: dict | None = None
    pressure_source_components: dict | None = None
    density_components: dict | None = None
    temperature_components: dict | None = None
    er_components: dict | None = None
