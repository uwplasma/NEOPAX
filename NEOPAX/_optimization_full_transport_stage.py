...
@dataclasses.dataclass
class DatabaseFullTransportFinalObjectiveOptimizationStage:
    """One persistent grouped terminal-objective VJP for an optimization stage.

    The benchmark creates its grouped objective closure for a single reverse
    evaluation.  Repeating that composition during optimization gives JAX a
    fresh transformed-function identity on every call.  Keep only the static
    runtime/objective structure here; the current final state and geometry
    leaves remain explicit numerical arguments.
    """

    geometry_layout: FloatingPayloadLeafLayout
    compiled_joint_vjp: Any
    geometry_floating_signature: Any
    final_state_signature: Any

    def joint_vjp_rows(self, *, final_y, geometry):
        self.geometry_layout.validate_static_structure(geometry)
        geometry_floating_leaves = self.geometry_layout.floating_leaves(geometry)
        if _tree_signature(geometry_floating_leaves) != self.geometry_floating_signature:
            raise ValueError(
                "Full-transport terminal-objective geometry shape, dtype, or "
                "weak type changed within a stage."
            )
        if _tree_signature(final_y) != self.final_state_signature:
            raise ValueError(
                "Full-transport terminal-objective final-state layout changed "
                "within a stage."
            )
        return self.compiled_joint_vjp(final_y, geometry_floating_leaves)

    def cache_size(self) -> int | None:
        cache_size = getattr(self.compiled_joint_vjp, "_cache_size", None)
        if not callable(cache_size):
            return None
        try:
            return int(cache_size())
...
def build_database_full_transport_final_objective_optimization_stage(
    *,
    runtime,
    reverse_setup,
    support_payload,
...
) -> DatabaseFullTransportFinalObjectiveOptimizationStage:
    """Build the optimization-owned JIT around the exact grouped VJP math."""

    # Imported lazily to avoid a module cycle.  These are the same small tree
    # and grouped-VJP helpers used by the benchmark body; this stage changes
    # cache ownership only.
    ...
    if not (
        ...
        raise ValueError(
            "The database terminal-objective optimization stage requires the "
            "live {geometry, database} support payload."
        )
    objective_indices = tuple(int(index) for index in objective_indices)
    if not objective_indices:
        raise ValueError("The grouped terminal-objective stage requires objectives.")
    geometry = support_payload["geometry"]
    geometry_layout = FloatingPayloadLeafLayout.from_template(geometry)
    geometry_floating_leaves = geometry_layout.floating_leaves(geometry)
    unpack_flat = reverse_setup.prepared_rollout.physics_context.unpack_flat
    runtime_template = runtime

    def _joint_vjp_kernel(active_final_y, active_geometry_floating_leaves):
        active_geometry = geometry_layout.rebuild(active_geometry_floating_leaves)
        geometry_delta0 = _float_delta_tree_like(active_geometry)
        ...
        def _objective_vector(final_y_value, geometry_delta):
            ...
            runtime_with_geometry = dataclasses.replace(
                runtime_template,
                geometry=_add_float_delta_tree(active_geometry, geometry_delta),
            )
            return jnp.stack(
                tuple(
                    objective_scalar_by_index(
        ...
        return _objective_vector_joint_vjp_rows(
            _objective_vector,
            active_final_y,
            geometry_delta0,
        )

    compiled_joint_vjp = jax.jit(_joint_vjp_kernel, inline=False)
    final_y = reverse_setup.prepared_rollout.initial_carry.y
    return DatabaseFullTransportFinalObjectiveOptimizationStage(
        geometry_layout=geometry_layout,
        compiled_joint_vjp=compiled_joint_vjp,
        geometry_floating_signature=_tree_signature(geometry_floating_leaves),
        final_state_signature=_tree_signature(final_y),
    )
