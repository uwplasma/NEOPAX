...
def _make_solver_state_transform(
    ...
    def unpack_flat_pullback(flat_y, state_bar):
        packed_state_like = unravel_packed(flat_y)
        if not (isinstance(packed_state_like, tuple) and len(packed_state_like) == 3):
            flat_bar, _ = jax.flatten_util.ravel_pytree(_pack_transport_state_arrays(state_bar, species))
            return flat_bar
        ...
        density_arr = jnp.asarray(density)
        pressure_arr = jnp.asarray(pressure)
        density_bar_out = jnp.asarray(getattr(state_bar, "density"))
        pressure_bar_out = jnp.asarray(getattr(state_bar, "pressure"))
        er_bar = jnp.asarray(getattr(state_bar, "Er"))
        ...
        floored_density = safe_density(full_density, density_floor)
        projected_pressure_pre = pressure_arr
        floored_density_bar = density_bar_out
        projected_pressure_pre_bar = pressure_bar_out
        if temperature_floor is not None:
            projected_temperature_pre = projected_pressure_pre / floored_density
            temp_floor_arr = _broadcast_species_floor(projected_temperature_pre, temperature_floor)
            active_temp_mask = projected_temperature_pre > temp_floor_arr
            floored_density_bar = floored_density_bar + jnp.where(active_temp_mask, 0.0, pressure_bar_out * temp_floor_arr)
            projected_pressure_pre_bar = jnp.where(active_temp_mask, pressure_bar_out, 0.0)
        ...
        if eidx is not None and packed_density.shape[-2] == template_state.density.shape[0] - 1:
            packed_density_bar = jnp.zeros_like(packed_density)
            packed_density_bar = packed_density_bar.at[..., :eidx, :].add(full_density_bar[..., :eidx, :])
            packed_density_bar = packed_density_bar.at[..., eidx:, :].add(full_density_bar[..., eidx + 1 :, :])
            charge_qp = jnp.asarray(species.charge_qp, dtype=pressure_arr.dtype)
            ion_indices = jnp.asarray(getattr(species, "ion_indices", ()), dtype=jnp.int32)
            if ion_indices.size > 0:
                Z_i = jnp.take(charge_qp, ion_indices, axis=0)
                electron_bar = full_density_bar[..., int(eidx), :]
                packed_density_bar = packed_density_bar.at[..., ion_indices, :].add(
                    electron_bar[..., None, :] * (-Z_i[..., None] / charge_qp[int(eidx)])
                )
        else:
            packed_density_bar = full_density_bar

        flat_in_bar, _ = jax.flatten_util.ravel_pytree((packed_density_bar, pressure_in_bar, er_bar))
        return flat_in_bar

    return flat_state0, unpack_flat, unpack_packed, pack_state, project_flat, project_flat_pullback, unpack_flat_pullback
...
def _shared_fluxes_hook(vector_field: Callable):
    ...
    if owner is None:
        return None
    eval_fn = getattr(owner, "evaluate_with_shared_fluxes", None)
    if callable(eval_fn):
        return eval_fn
    return None
...
def _flat_rhs_with_shared_fluxes_factory(unravel, vector_field, args, kwargs, project_flat=None):
    species = _extract_species_from_args(args)
    eval_fn = _shared_fluxes_hook(vector_field)

    def _flat_rhs(t_value, flat_y, shared_fluxes):
        projected_flat_y = _project_flat_state_if_needed(
            flat_y,
            project_flat,
        )
        state_y = unravel(projected_flat_y)
        if eval_fn is not None:
            rhs_tree = eval_fn(t_value, state_y, *args, shared_fluxes=shared_fluxes, **kwargs)
        else:
            raise ValueError(
                "Lagged shared-flux RHS mode requires a vector field owner that defines "
                "evaluate_with_shared_fluxes(...)."
            )
        rhs_flat, _ = jax.flatten_util.ravel_pytree(_pack_transport_state_arrays(rhs_tree, species))
        return rhs_flat
...
@jax.tree_util.register_dataclass
@dataclasses.dataclass(frozen=True, eq=False)
class _RadauAcceptedStepReplayCarryTrace:
    t: Any
    y: Any
    dt: Any
    prev_stages: Any
...
def _radau_tree_l2_norm(tree):
    return jnp.sqrt(
        jnp.maximum(
            _radau_tree_vdot(tree, tree),
...
@jax.tree_util.register_dataclass
@dataclasses.dataclass(frozen=True, eq=False)
class _RadauAcceptedStepForwardLikeCotangent:
    """Reverse propagated state chosen to mirror the forward accepted-step contract."""
    y: Any
    dt: Any
    prev_stages: Any
    lagged_reference_y: Any
...
@jax.tree_util.register_dataclass
@dataclasses.dataclass(frozen=True, eq=False)
class _RadauAcceptedStepForwardLikeNoStageCotangent:
    """Smaller forward-like reverse state with no propagated stage-history lane."""
    y: Any
    dt: Any
    lagged_reference_y: Any
...
def _radau_backward_payload_candidate(
    ...
    This keeps the predictor / lagged-response context and the cached
    Jacobian/LU reuse state that the accepted-step primal actually reads.
    Controller bookkeeping is still excluded.
    """
...
def _radau_replay_state_tangent_from_primal_linearized_reuse_only(
    ...
    """Reduced replay-state tangent map for the reuse-only local reverse path.

    This mirrors the same approximate accepted-step tangent map used in forward
    mode, but exposes the replay-state outputs needed by the realized-schedule
    reverse:
    - accepted `y`
    - `prev_stages`
    - `lagged_reference_y`
...
def _radau_stage_evals_from_shared_fluxes_factory(
    ...
    if physics_context.flat_rhs_with_shared_fluxes is None:
        return None
    stages_final = primal_result.stage_history.reshape((kernel_context.num_stages, kernel_context.state_dim))
    stage_times = carry_in.t + kernel_context.c * primal_result.trial_dt
    stage_states = carry_in.y[None, :] + primal_result.trial_dt * (kernel_context.a @ stages_final)

    def _stage_evals_from_shared_fluxes(shared_fluxes_value):
        return jax.vmap(
            lambda t_eval, y_eval, fluxes_eval: physics_context.flat_rhs_with_shared_fluxes(
                t_eval,
                y_eval,
                fluxes_eval,
            ),
            in_axes=(0, 0, 0),
        )(stage_times, stage_states, shared_fluxes_value)

    return _stage_evals_from_shared_fluxes


def _radau_stage_shared_fluxes_from_flux_response_factory(
    kernel_context: _RadauAcceptedStepKernelContext,
    physics_context: _RadauAcceptedStepPhysicsContext,
    carry_in: _RadauAcceptedStepCarry,
    primal_result: _RadauAcceptedStepAttemptResult,
    ...
    shared_flux_model = None if owner is None else getattr(owner, "shared_flux_model", None)
    ...
    def _stage_shared_fluxes_from_flux_response(flux_response):
        def _eval_one(y_eval):
            state_y = physics_context.unpack_flat(
                _project_flat_state_if_needed(y_eval, physics_context.project_flat)
            )
            return shared_flux_model.evaluate_with_lagged_response(
                state_y,
                flux_response,
            )

        return jax.vmap(_eval_one)(stage_states)

    return _stage_shared_fluxes_from_flux_response


def _radau_stage_flux_response_pullback_factory(
    kernel_context: _RadauAcceptedStepKernelContext,
    physics_context: _RadauAcceptedStepPhysicsContext,
    carry_in: _RadauAcceptedStepCarry,
    primal_result: _RadauAcceptedStepAttemptResult,
    ...
    if shared_flux_model is None:
        return None
    pullback_fn = getattr(shared_flux_model, "pullback_evaluate_with_lagged_response", None)
    ...
    def _pullback_one(y_eval, flux_response_value, shared_flux_bar_eval):
        state_y = physics_context.unpack_flat(
            _project_flat_state_if_needed(y_eval, physics_context.project_flat)
        )
    ...
    *,
    ...
    if isinstance(lagged_response, TransportLaggedResponse):
        if flux_response_pullback_fn is not None:
            ...
            (flux_response_bar,) = flux_pullback(stage_rhs_bar)
        else:
...
def _radau_contract_reduced_output_bar(
    reduced_output_bar: _RadauAcceptedStepReducedOutput,
    mode: str = "full",
) -> _RadauAcceptedStepReducedOutput:
    """Contract the propagated reverse cotangent toward the forward JVP boundary.

    `full` preserves the current reverse rollout contract.

    `forward-like-v1` is the first conservative contraction pass:
    - keep `y_out`, `dt_out`, and `prev_stages_out` active
    - zero rollout-level lanes that forward mode does not treat as first-class
      tangent inputs:
      - `prev_dt_out`
      - `lagged_reference_y_out`
      - `prev_theta_final_out`

    This keeps the accepted-step local pullback unchanged while starting to
    reduce the multi-step propagated cotangent width.
    """
    if mode == "full":
        return reduced_output_bar
    if mode == "forward-like-v1":
        return dataclasses.replace(
            reduced_output_bar,
...
def _radau_forward_like_cotangent_from_reduced_output_bar(
    reduced_output_bar: _RadauAcceptedStepReducedOutput,
    template_carry: _RadauAcceptedStepCarry,
) -> _RadauAcceptedStepForwardLikeCotangent:
    """Map the broader reduced-output cotangent into a forward-like reverse contract."""
    return _RadauAcceptedStepForwardLikeCotangent(
        ...
        prev_stages=jnp.asarray(reduced_output_bar.prev_stages_out),
        lagged_reference_y=jnp.asarray(reduced_output_bar.lagged_reference_y_out),
    )


def _radau_forward_like_cotangent_to_reduced_output_bar(
    cotangent: _RadauAcceptedStepForwardLikeCotangent,
    template_carry: _RadauAcceptedStepCarry,
) -> _RadauAcceptedStepReducedOutput:
    """Expand the forward-like reverse contract into the broader local pullback shape."""
    return _RadauAcceptedStepReducedOutput(
        t_out=jnp.zeros_like(template_carry.t),
        y_out=jnp.asarray(cotangent.y),
        dt_out=jnp.asarray(cotangent.dt),
        prev_stages_out=jnp.asarray(cotangent.prev_stages),
...
def _radau_forward_like_no_stage_cotangent_from_reduced_output_bar(
    reduced_output_bar: _RadauAcceptedStepReducedOutput,
) -> _RadauAcceptedStepForwardLikeNoStageCotangent:
    return _RadauAcceptedStepForwardLikeNoStageCotangent(
        y=jnp.asarray(reduced_output_bar.y_out),
        dt=jnp.asarray(reduced_output_bar.dt_out) + jnp.asarray(reduced_output_bar.prev_dt_out),
        lagged_reference_y=jnp.asarray(reduced_output_bar.lagged_reference_y_out),
    )
...
def _radau_forward_like_no_stage_cotangent_to_reduced_output_bar(
    ...
    return _RadauAcceptedStepReducedOutput(
        ...
        y_out=jnp.asarray(cotangent.y),
        dt_out=jnp.asarray(cotangent.dt),
        prev_stages_out=jnp.zeros_like(template_carry.prev_stages),
        prev_dt_out=jnp.zeros_like(template_carry.prev_dt),
        lagged_reference_y_out=jnp.asarray(cotangent.lagged_reference_y),
        prev_theta_final_out=jnp.zeros_like(template_carry.prev_theta_final),
    )
...
def _radau_accepted_step_forward_like_pullback(
    kernel_context: _RadauAcceptedStepKernelContext,
    physics_context: _RadauAcceptedStepPhysicsContext,
    template_carry: _RadauAcceptedStepCarry,
    attempt_context: _RadauAcceptedStepAttemptContext,
    ...
    """Reverse helper that propagates a forward-like accepted-step contract.

    This mirrors the forward accepted-step tangent contract more directly than
    `_radau_accepted_step_primitive_pullback(...)` by:

    - taking only the forward-like propagated lanes as cotangent inputs
    - using the same reduced accepted-step linearization that forward mode uses
    - branching the lagged cache transpose the same way forward branches the
      lagged tangent
    """
    carry_in = _radau_carry_from_reverse_payload(
        reverse_payload,
    ...
    dy_bar, dh_bar, _dt_in_bar, lagged_response_cache_bar, stage_rhs_bar = _radau_accepted_step_y_pullback_linearized(
        kernel_context,
        physics_context,
        carry_in,
        primal_result,
    ...
    if not kernel_context.use_transport_lagged_response or physics_context.build_lagged_response is None:
        ...
        lagged_response_cache_in_bar, lagged_reference_y_in_bar, dy_extra = jax.lax.cond(
            carry_in.lagged_response_valid,
            _reuse_case,
            _rebuild_case,
            operand=None,
    ...
    return _RadauAcceptedStepForwardLikeCotangent(
        ...
        prev_stages=jnp.asarray(prev_stages_in_bar.reshape((-1,)), dtype=kernel_context.dtype),
        lagged_reference_y=jnp.asarray(lagged_reference_y_in_bar, dtype=kernel_context.dtype),
    )


def _radau_accepted_step_forward_like_no_stage_pullback(
    kernel_context: _RadauAcceptedStepKernelContext,
    physics_context: _RadauAcceptedStepPhysicsContext,
    template_carry: _RadauAcceptedStepCarry,
    attempt_context: _RadauAcceptedStepAttemptContext,
    ...
    """Forward-like reverse helper with no propagated stage-history cotangent lane."""
    carry_in = _radau_carry_from_reverse_payload(
        reverse_payload,
        template_carry,
        physics_context,
    ...
    accepted_y_bar = jnp.asarray(cotangent.y, dtype=kernel_context.dtype)
    dy_bar, dh_bar, _dt_in_bar, lagged_response_cache_bar, _stage_rhs_bar = _radau_accepted_step_y_pullback_linearized(
        kernel_context,
        physics_context,
        carry_in,
        primal_result,
    ...
    def _scan_body(carry, xs):
        ...
        def _do_attempt(_):
            ...
            def _rejected_attempt(__):
                ...
                # Rejected attempts only update replay caches/controller state.
                # They are intentionally nondifferentiated here, so avoid tracing
                # the accepted-step custom-JVP path for this branch.
                attempt_result = _execute_radau_accepted_step_attempt(
                    execution_context.kernel_context,
                    execution_context.physics_context,
                    _radau_carry_with_forward_only_jvp_fields(carry_for_step),
...
def _radau_replay_realized_attempt_final_carry(
    execution_context: _RadauSolveExecutionContext,
    carry0: _RadauAcceptedStepCarry,
    active_mask,
    accepted_mask,
    ...
    """Replay the full realized attempt history and return only the final carry.

    This is the lighter-weight replay path used by the default reverse sweep,
    which does not need the per-attempt diagnostic arrays.
    """
    ...
    def _scan_body(carry, xs):
        ...
        return jax.lax.cond(active, _do_attempt, lambda __: carry, operand=None), None
...
def _radau_forward_replay_realized_accepted_attempt_rollout(
    execution_context: _RadauSolveExecutionContext,
    carry0: _RadauAcceptedStepCarry,
    accepted_active_mask,
    accepted_dts,
    ...
    """Forward-benchmark-only replay of realized accepted attempts.

    Keep this lane independent from reverse-mode replay helpers so benchmarked
    forward custom-JVP behavior can remain stable even if reverse replay logic
    evolves.
    """
...
def _radau_replay_realized_accepted_carry_pullback(
    ...
    next_easy_growth_streak,
    next_lagged_response_valid,
    carry_trace: _RadauAcceptedStepReplayCarryTrace,
    final_replay_state_bar,
):
    ...
    def _reverse_body(replay_state_cotangent, inputs):
        ...
        carry_before_base = _RadauAcceptedStepCarry(
            ...
            dt=carry_trace.dt[step_index],
            prev_error=jnp.zeros_like(carry0.prev_error),
            prev_stages=carry_trace.prev_stages[step_index],
            prev_dt=carry_trace.prev_dt[step_index],
        ...
        replay_state_before = _radau_replay_state_from_carry(carry_before_base)
        ...
        if replay_output_diagnostic in {None, "y"}:
            carry_before = carry_before_base
        ...
        if (
            ...
            def _print_step_pullback_check(_):
                carry_before_for_check = carry_before_base
                ...
                lagged_response_for_check, _, _ = _radau_prepare_lagged_response(
                    execution_context.kernel_context,
                    carry_before_for_check,
                ...
                rhs_time_ref_for_check = jax.jacfwd(
                    lambda t_eval: _radau_eval_rhs(
                        t_eval,
                        carry_before_for_check.y,
                        lagged_response_for_check,
                ...
                direct_dy_bar_check, direct_dh_bar_check, direct_dt_in_bar_check, _direct_lagged_cache_bar_check = (
                    _radau_accepted_step_y_pullback_linearized(
                        execution_context.kernel_context,
                        execution_context.physics_context,
                        carry_before_for_check,
                        ...
                        lagged_response_for_check,
                        rhs_time_ref_for_check,
                        zero_lagged_cache_tangent=_radau_zero_cotangent_like(
                ...
                jax.debug.print(
                    ...
                    "direct_dy_l2={direct_dy_l2:.6e} replay_dy_l2={replay_dy_l2:.6e} "
                    "direct_vs_replay_dy_diff_l2={direct_diff_l2:.6e} "
                    "direct_dh={direct_dh:.6e} replay_t_bar={replay_t_bar:.6e} direct_dt_in={direct_dt_in:.6e}",
                    seg=segment_index,
                    step=step_index,
                    ...
                    rhs=rhs,
                    err=jnp.abs(lhs - rhs),
                    direct_dy_l2=jnp.linalg.norm(jnp.ravel(jnp.asarray(direct_dy_bar_check, dtype=dtype))),
                    replay_dy_l2=jnp.linalg.norm(jnp.ravel(jnp.asarray(carry_before_bar.y, dtype=dtype))),
                    direct_diff_l2=jnp.linalg.norm(
                        jnp.ravel(
                            jnp.asarray(carry_before_bar.y, dtype=dtype)
                            - jnp.asarray(direct_dy_bar_check, dtype=dtype)
                        )
                    ),
                    direct_dh=jnp.asarray(direct_dh_bar_check, dtype=dtype),
                    replay_t_bar=jnp.asarray(carry_before_bar.t, dtype=dtype),
                    direct_dt_in=jnp.asarray(direct_dt_in_bar_check, dtype=dtype),
                    ordered=True,
                )
                return jnp.asarray(0, dtype=jnp.int32)
...
def _radau_pad_schedule_array(values, *, target_len: int):
    pad_len = int(target_len) - int(values.shape[0])
    if pad_len <= 0:
        return values
    pad_config = [(0, pad_len)] + [(0, 0)] * (values.ndim - 1)
    return jnp.pad(values, pad_config)


def _radau_segment_schedule_array(values, *, segment_length: int):
    total_len = int(values.shape[0])
    n_segments = (total_len + int(segment_length) - 1) // int(segment_length)
    padded_len = n_segments * int(segment_length)
    padded = _radau_pad_schedule_array(values, target_len=padded_len)
    return padded.reshape((n_segments, int(segment_length)) + padded.shape[1:]), n_segments


def _radau_segmented_schedule_arrays(
    active_mask,
    attempted_dts,
    next_dts,
    next_recent_reject_count,
    ...
    *,
    ...
    segmented_active_mask, n_segments = _radau_segment_schedule_array(
        active_mask, segment_length=segment_length
    )

    def _reshape(values):
        segmented, _ = _radau_segment_schedule_array(values, segment_length=segment_length)
        return segmented

    return (
        segmented_active_mask,
        _reshape(attempted_dts),
        _reshape(next_dts),
        _reshape(next_recent_reject_count),
        _reshape(next_regrowth_cooldown),
        _reshape(next_easy_growth_streak),
        _reshape(next_lagged_response_valid),
        n_segments,
    )


def _radau_replay_realized_segment_final_carry(
    execution_context: _RadauSolveExecutionContext,
    carry0: _RadauAcceptedStepCarry,
    segment_active_mask,
    ...
    def _step(carry, xs):
        ...
        return jax.lax.cond(active, _do_step, lambda _: carry, operand=None)
...
def _radau_replay_realized_segment_payloads(
    execution_context: _RadauSolveExecutionContext,
    carry0: _RadauAcceptedStepCarry,
    segment_active_mask,
    segment_attempted_dts,
    ...
    def _scan_body(carry, xs):
        ...
        def _do_step(_):
            ...
            payload = _radau_backward_payload_candidate(carry, attempt_result)
            return next_carry, payload

        def _skip(_):
            return carry, _inactive_payload(carry)

        return jax.lax.cond(active, _do_step, _skip, operand=None)

    final_carry, payloads = jax.lax.scan(
        _scan_body,
        carry0,
        (
            segment_active_mask,
    ...
    return final_carry, payloads


def _radau_replay_realized_accepted_carry_trace(
    execution_context: _RadauSolveExecutionContext,
    carry0: _RadauAcceptedStepCarry,
    accepted_active_mask,
    accepted_dts,
    ...
    """Replay accepted steps once and record the primal carry-before trace.

    This trace is used as nondifferentiated primal context for the accepted-step
    reverse sweep. It is much cheaper than the old payload tape and avoids the
    nested prefix-replay graph.
    """

    dtype = execution_context.dtype

    def _trace_from_carry(carry):
        return _RadauAcceptedStepReplayCarryTrace(
            t=carry.t,
            y=carry.y,
            dt=carry.dt,
            prev_stages=carry.prev_stages,
    ...
    def _scan_body(carry, xs):
        ...
        trace_entry = _trace_from_carry(carry)
        ...
        next_carry = jax.lax.cond(active, _do_step, lambda _: carry, operand=None)
        return next_carry, trace_entry
    ...
    trace = jax.tree_util.tree_map(jax.lax.stop_gradient, trace)
    return final_carry, trace


def _radau_replay_realized_checkpoint_carries(
    execution_context: _RadauSolveExecutionContext,
    carry0: _RadauAcceptedStepCarry,
    segmented_active_mask,
    ...
    def _segment_body(carry, segment_xs):
        ...
        next_carry = _radau_replay_realized_segment_final_carry(
            execution_context,
            carry,
            *segment_xs,
        )
...
def _radau_replay_realized_attempt_checkpoint_carries(
    execution_context: _RadauSolveExecutionContext,
    carry0: _RadauAcceptedStepCarry,
    segmented_active_mask,
...
def _radau_adaptive_final_state_rollout(
    ...
    *,
    ...
    This matches the older forward benchmark contract more closely than the
    lighter schedule-only helper: primal accepted/rejected behavior still
    follows the production adaptive controller, but the saved trace is limited
    to the compact metadata needed by the custom forward JVP/VJP.
    """
    ...
    def _scan_body_with_idx(step_state, step_idx):
        ...
        next_step_state, step_info, _payload = jax.lax.cond(active, _run, _skip, operand=None)
        scan_out = (
            active,
            jnp.asarray(step_info.accepted),
            jnp.asarray(step_info.err_norm, dtype=dtype),
            jnp.asarray(
                0.0 if getattr(step_info, "theta_final", None) is None else getattr(step_info, "theta_final"),
                dtype=dtype,
            ),
            jnp.asarray(
                0 if getattr(step_info, "newton_iter_count", None) is None else getattr(step_info, "newton_iter_count"),
                dtype=jnp.int32,
            ),
            jnp.asarray(next_step_state.cache_valid),
            jnp.asarray(step_info.dt, dtype=dtype),
...
def _radau_forward_adaptive_final_state_rollout(
    execution_context: _RadauSolveExecutionContext,
    carry0: _RadauAcceptedStepCarry,
    *,
    max_total_steps: int,
    ...
    """Forward-only adaptive rollout for benchmarked custom-JVP paths.

    This lane intentionally avoids reverse payload construction and keeps only
    the compact schedule/controller metadata required by the forward replay.
    """
...
@partial(jax.custom_jvp, nondiff_argnums=(0, 1, 2))
def _radau_forward_adaptive_final_y_realized_schedule(
    execution_context: _RadauSolveExecutionContext,
    max_total_steps: int,
    stop_after_accepted_steps: int | None,
    carry0: _RadauAcceptedStepCarry,
):
    """Forward-only final adaptive state with a realized-schedule JVP."""
...
@_radau_forward_adaptive_final_y_realized_schedule.defjvp
def _radau_forward_adaptive_final_y_realized_schedule_jvp(
    execution_context: _RadauSolveExecutionContext,
    max_total_steps: int,
    stop_after_accepted_steps: int | None,
    primals,
...
def _radau_adaptive_final_y_realized_schedule_vjp_fwd(
    ...
    segmented_full_accepted_mask, _ = _radau_segment_schedule_array(
        full_accepted_mask,
        segment_length=checkpoint_interval,
    )
    residuals = (
...
def _radau_adaptive_final_y_realized_schedule_vjp_bwd(
    ...
    if reverse_device is not None:
        ...
        segmented_attempted_dts = jax.device_put(segmented_attempted_dts, reverse_device)
        segmented_next_dts = jax.device_put(segmented_next_dts, reverse_device)
        segmented_next_recent_reject_count = jax.device_put(segmented_next_recent_reject_count, reverse_device)
        segmented_next_regrowth_cooldown = jax.device_put(segmented_next_regrowth_cooldown, reverse_device)
        segmented_next_easy_growth_streak = jax.device_put(segmented_next_easy_growth_streak, reverse_device)
...
def _build_prepared_radau_accepted_rollout(
    *,
    ...
    temperature_active_mask, fixed_temperature_profile = _extract_fixed_temperature_projection(vector_field)
    density_floor, temperature_floor = _extract_state_regularization(vector_field)
    flat_state0, unpack_flat, _unpack_packed, _pack_state, project_flat, project_flat_pullback, unpack_flat_pullback = _make_solver_state_transform(
        state,
        species,
        temperature_active_mask=temperature_active_mask,
        fixed_temperature_profile=fixed_temperature_profile,
    ...
    flat_rhs_with_shared_fluxes_raw = _flat_rhs_with_shared_fluxes_factory(
        unravel=unpack_flat,
        vector_field=vector_field,
        args=args,
        kwargs=kwargs,
    ...
    if build_lagged_response is None:
        ...
        def build_lagged_response_pullback(flat_y_source, lagged_response_bar):
            ...
            projected_flat_y = _project_flat_state_if_needed(flat_y_source, project_flat)
            projected_flat_bar = unpack_flat_pullback(projected_flat_y, state_bar)
            if project_flat is None:
                return projected_flat_bar
            return project_flat_pullback(flat_y_source, projected_flat_bar)
...
class ThetaMethodSolver(_ThetaSolverConfig):
    def solve(self, state, vector_field: Callable, *args, **kwargs):
        ...
        flat_state0, unpack_flat, _unpack_packed, _pack_state, project_flat, _project_flat_pullback, unpack_flat_pullback = _make_solver_state_transform(
            state,
            species,
            temperature_active_mask=temperature_active_mask,
            fixed_temperature_profile=fixed_temperature_profile,
