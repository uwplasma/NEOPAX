...
def _lagged_response_flux_state_pullback_generic_hook(vector_field: Callable):
    owner = getattr(vector_field, "__self__", None)
    if owner is None:
        return None
    pullback_fn = getattr(owner, "pullback_evaluate_with_lagged_response_state_flux_generic", None)
    return pullback_fn if callable(pullback_fn) else None


def _lagged_response_joint_generic_state_pullback_hook(vector_field: Callable):
    owner = getattr(vector_field, "__self__", None)
    if owner is None:
        return None
    pullback_fn = getattr(owner, "pullback_evaluate_with_lagged_response_state_joint_generic", None)
    return pullback_fn if callable(pullback_fn) else None
...
def _exact_stage_transpose_solve_compact_hook(vector_field: Callable):
    ...
    if owner is None:
        return None
    solve_fn = getattr(owner, "solve_exact_stage_transpose_compact", None)
...
def _flat_rhs_build_support_pullback_batched_interpolated_faces_factory(
    ...
    elif native_multi_rhs_reuse_moment_drds_jvp_shared_primal_with_vmec_coefficients_direct_directional_product_rule:
        pullback_name = (
            "pullback_build_lagged_response_support_payload_batched_interpolated_faces_"
            "native_multi_rhs_reuse_moment_drds_jvp_shared_primal_with_vmec_coefficients_"
            "direct_directional_product_rule"
        )
    elif native_multi_rhs_reuse_moment_drds_jvp_shared_primal_with_vmec_coefficients:
        pullback_name = (
            "pullback_build_lagged_response_support_payload_batched_interpolated_faces_"
            "native_multi_rhs_reuse_moment_drds_jvp_shared_primal_with_vmec_coefficients"
        )
    elif native_multi_rhs_compact_residual_reuse_moment_drds_jvp_shared_primal:
        pullback_name = (
            "pullback_build_lagged_response_support_payload_batched_interpolated_faces_"
            "native_multi_rhs_compact_residual_reuse_moment_drds_jvp_shared_primal"
        )
    elif native_multi_rhs_reuse_moment_drds_jvp_shared_primal:
        pullback_name = (
            "pullback_build_lagged_response_support_payload_batched_interpolated_faces_"
            "native_multi_rhs_reuse_moment_drds_jvp_shared_primal"
        )
    elif native_multi_rhs_compact_shared_primal:
        pullback_name = (
            "pullback_build_lagged_response_support_payload_batched_interpolated_faces_"
    ...
    elif multi_rhs_shared_primal:
        pullback_name = (
            "pullback_build_lagged_response_support_payload_batched_interpolated_faces_"
            "multi_rhs_shared_primal"
        )
    elif reuse_local_vjp_primal:
        pullback_name = (
            "pullback_build_lagged_response_support_payload_batched_interpolated_faces_"
            "reuse_local_vjp_primal"
        )
    else:
        pullback_name = "pullback_build_lagged_response_support_payload_batched_interpolated_faces"
    pullback_fn = getattr(owner, pullback_name, None)
    if pullback_fn is None:
        return None
...
def _flat_rhs_split_state_pullback_factory(
    ...
    *,
    ...
    if component == "direct":
        ...
        pullback_fn = _lagged_response_flux_state_pullback_generic_hook(vector_field)
    elif component == "joint_generic":
        pullback_fn = _lagged_response_joint_generic_state_pullback_hook(vector_field)
    else:
...
class _RadauAcceptedStepPhysicsContext:
    ...
    flat_rhs_build_support_pullback_batched_interpolated_faces_native_multi_rhs_reuse_moment_drds_jvp_shared_primal: Callable[[Any, Any, Any], Any] | None = None
...
def _radau_finish_native_vmec_rebuild_from_common(
    ...
    if lagged_response_branch == "rebuild":
        ...
        native_pullback = (
            ...
            else physics_context.flat_rhs_build_support_pullback_batched_interpolated_faces_native_multi_rhs_reuse_moment_drds_jvp_shared_primal_with_vmec_coefficients_direct_directional_product_rule
            if rebuild_support_mode == (
            ...
            else physics_context.flat_rhs_build_support_pullback_batched_interpolated_faces_native_multi_rhs_reuse_moment_drds_jvp_shared_primal_with_vmec_coefficients
        )
...
def _execute_radau_accepted_step_next_reduced_cotangent_batched_bwd_with_support_from_primal_result(
    ...
    if lagged_response_branch == "rebuild":
        ...
        if (
            ...
            if rebuild_support_pullback_mode in {
                ...
                if rebuild_support_pullback_mode == (
                    ...
                    batched_pullback = (
                        physics_context.flat_rhs_build_support_pullback_batched_interpolated_faces_native_multi_rhs_compact_residual_reuse_moment_drds_jvp_shared_primal
                    )
                elif rebuild_support_pullback_mode == (
                    "ntx_batched_interpolated_faces_native_multi_rhs_reuse_moment_drds_jvp_shared_primal"
                ):
                    ...
                    batched_pullback = (
                        physics_context.flat_rhs_build_support_pullback_batched_interpolated_faces_native_multi_rhs_shared_primal
                    )
                elif rebuild_support_pullback_mode == (
                    "ntx_batched_interpolated_faces_multi_rhs_shared_primal"
                ):
                    batched_pullback = (
                        physics_context.flat_rhs_build_support_pullback_batched_interpolated_faces_multi_rhs_shared_primal
                    )
                elif rebuild_support_pullback_mode == (
                    "ntx_batched_interpolated_faces_reuse_local_vjp_primal"
                ):
                    batched_pullback = (
                        physics_context.flat_rhs_build_support_pullback_batched_interpolated_faces_reuse_local_vjp_primal
                    )
                else:
                    batched_pullback = (
                ...
                if batched_pullback is None:
                    raise RuntimeError(
                        "the requested batched NTX rebuild support pullback was requested, "
                        "but the active transport physics context does not expose that hook."
                    )
...
def _radau_zero_native_vmec_face_coefficient_bars(support):
    """Zero parallel VMEC-face coefficient bars for the opt-in native mode.

    These are deliberately not inserted into ``support``: they are
    cotangents of the traceable VMEC surface coefficients, rather than of a
    primal support-payload leaf.  Keeping them as appended carrier leaves lets
    the established Radau scan accumulate them without changing any default
    support-tree contract.
    """

    ntx_support = support["ntx_support"] if isinstance(support, dict) else support
    surface = ntx_support.face_prepared.surface
    return {
        name: jnp.zeros_like(jnp.asarray(getattr(surface, name)))
        for name in (
            "b_cos",
            "jacobian_cos",
            "b_sub_theta_cos",
...
def _radau_exact_stage_residual_transpose_matvec_diagnostic(
    ...
    *,
    ...
    generic_flux_jt_lambda_stages = None
    if physics_context.flat_rhs_flux_state_pullback_generic is not None:
        generic_flux_jt_lambda_stages = jax.vmap(
            lambda t_eval, y_eval, lambda_eval: physics_context.flat_rhs_flux_state_pullback_generic(
                t_eval,
                y_eval,
                lagged_response,
                lambda_eval,
    ...
    joint_generic_jt_lambda_stages = None
    if physics_context.flat_rhs_joint_generic_state_pullback is not None:
        joint_generic_jt_lambda_stages = jax.vmap(
    ...
    compact_flux_vs_generic_flux_diff = (
        flux_jt_lambda_stages - generic_flux_jt_lambda_stages
    ...
    joint_generic_diff = (
        joint_generic_jt_lambda_stages - generic_jt_lambda_stages
    ...
    compact_flux_vs_generic_flux_pressure_diff_l2 = (
    ...
    off_tridiagonal_l2 = jnp.sqrt(
        jnp.maximum(
            jnp.vdot(off_tridiagonal_blocks, off_tridiagonal_blocks),
            jnp.asarray(0.0, dtype=kernel_context.dtype),
        )
    )
    block_l2 = jnp.sqrt(
    ...
    return {
        ...
        "joint_generic_diff_l2": joint_generic_diff_l2,
        "joint_generic_pressure_diff_l2": joint_generic_pressure_diff_l2,
        ...
        "radial_variables_per_cell": jnp.asarray(layout.variables_per_cell, dtype=jnp.int32),
        "radial_extra_per_cell_count": jnp.asarray(layout.extra_per_cell_count, dtype=jnp.int32),
        "radial_off_tridiagonal_l2": off_tridiagonal_l2,
        "radial_off_tridiagonal_rel_l2": off_tridiagonal_l2 / jnp.maximum(
            matrix_l2,
            jnp.asarray(jnp.finfo(kernel_context.dtype).tiny, dtype=kernel_context.dtype),
        ),
...
def _radau_block_tridiagonal_solve(lower, diagonal, upper, rhs, *, transpose: bool = False):
    """Solve a block-tridiagonal linear system with one or more RHS columns.

    The block-row convention is

        lower[k] @ x[k - 1] + diagonal[k] @ x[k] + upper[k] @ x[k + 1] = rhs[k]

    with ``lower[0]`` and ``upper[-1]`` unused.  If ``transpose`` is true, solve
    the transpose system without materializing a dense global matrix.
    """
    lower = jnp.asarray(lower)
    diagonal = jnp.asarray(diagonal)
    upper = jnp.asarray(upper)
    rhs_arr = jnp.asarray(rhs, dtype=diagonal.dtype)
    squeeze_rhs = rhs_arr.ndim == 2
    if squeeze_rhs:
        rhs_arr = rhs_arr[..., None]

    if transpose:
        lower_t = jnp.concatenate(
            [jnp.zeros_like(lower[:1]), jnp.swapaxes(upper[:-1], -1, -2)],
            axis=0,
        )
        diagonal_t = jnp.swapaxes(diagonal, -1, -2)
        upper_t = jnp.concatenate(
            [jnp.swapaxes(lower[1:], -1, -2), jnp.zeros_like(upper[-1:])],
            axis=0,
        )
        lower, diagonal, upper = lower_t, diagonal_t, upper_t

    n_blocks = int(diagonal.shape[0])
    block_dim = int(diagonal.shape[-1])
    zero_c = jnp.zeros((block_dim, block_dim), dtype=diagonal.dtype)

    lu0, piv0 = jax.scipy.linalg.lu_factor(diagonal[0])
    c0 = jax.lax.cond(
        n_blocks > 1,
        lambda _: jax.scipy.linalg.lu_solve((lu0, piv0), upper[0]),
        lambda _: zero_c,
        operand=None,
    )
    d0 = jax.scipy.linalg.lu_solve((lu0, piv0), rhs_arr[0])

    def _forward(carry, k):
        c_prev, d_prev = carry
        denom = diagonal[k] - lower[k] @ c_prev
        lu_k, piv_k = jax.scipy.linalg.lu_factor(denom)
        c_k = jax.lax.cond(
            k < n_blocks - 1,
            lambda _: jax.scipy.linalg.lu_solve((lu_k, piv_k), upper[k]),
            lambda _: zero_c,
            operand=None,
        )
        d_k = jax.scipy.linalg.lu_solve((lu_k, piv_k), rhs_arr[k] - lower[k] @ d_prev)
        return (c_k, d_k), (c_k, d_k)

    if n_blocks > 1:
        (_, _), (c_tail, d_tail) = jax.lax.scan(
            _forward,
            (c0, d0),
            jnp.arange(1, n_blocks, dtype=jnp.int32),
        )
        c_all = jnp.concatenate([c0[None, ...], c_tail], axis=0)
        d_all = jnp.concatenate([d0[None, ...], d_tail], axis=0)
    else:
        c_all = c0[None, ...]
        d_all = d0[None, ...]
    ...
    if n_blocks > 1:
        _, x_rev_tail = jax.lax.scan(
            _backward,
            x_last,
            jnp.arange(n_blocks - 2, -1, -1, dtype=jnp.int32),
        )
        solution = jnp.concatenate([jnp.flip(x_rev_tail, axis=0), x_last[None, ...]], axis=0)
    else:
        solution = x_last[None, ...]
...
def _radau_infer_radial_block_layout(kernel_context: _RadauAcceptedStepKernelContext) -> _RadauRadialBlockLayout:
    ...
    if n_radial <= 0:
        raise ValueError("Invalid radial block layout: inferred non-positive radial size.")
    if state_dim % n_radial != 0:
        raise ValueError(
    ...
    er_count = er_size // n_radial
    if er_count not in (0, 1):
        raise ValueError(
            "Expected at most one Er unknown per radial cell in the packed Radau state; "
            f"got {er_count}."
        )
    extra_per_cell_count = extra_size // n_radial
    variables_per_cell = density_species_count + pressure_species_count + er_count + extra_per_cell_count
    if variables_per_cell * n_radial != state_dim:
        raise ValueError(
            "Inferred radial block layout does not reproduce the packed state size; "
            f"state_dim={state_dim}, n_radial={n_radial}, variables_per_cell={variables_per_cell}, "
            f"density_species_count={density_species_count}, "
            f"pressure_species_count={pressure_species_count}, er_count={er_count}, "
            f"extra_per_cell_count={extra_per_cell_count}."
        )
    return _RadauRadialBlockLayout(
        n_radial=n_radial,
...
def _radau_stage_stack_to_radial_blocks(
    kernel_context: _RadauAcceptedStepKernelContext,
    stage_stack,
):
    """Convert ``(num_stages, state_dim)`` data to radial block ordering."""
    layout = _radau_infer_radial_block_layout(kernel_context)
    stage_stack = jnp.asarray(stage_stack, dtype=kernel_context.dtype).reshape(
        (kernel_context.num_stages, kernel_context.state_dim)
    )
    density_end = int(kernel_context.density_size)
    pressure_end = density_end + int(kernel_context.pressure_size)
    ...
    if layout.density_species_count:
        density = stage_stack[:, :density_end].reshape(
            (kernel_context.num_stages, layout.density_species_count, layout.n_radial)
        )
        pieces.append(jnp.swapaxes(density, 1, 2))
    if layout.pressure_species_count:
        pressure = stage_stack[:, density_end:pressure_end].reshape(
            (kernel_context.num_stages, layout.pressure_species_count, layout.n_radial)
        )
        pieces.append(jnp.swapaxes(pressure, 1, 2))
    if layout.has_er:
        er = stage_stack[:, pressure_end:er_end].reshape(
            (kernel_context.num_stages, 1, layout.n_radial)
        )
        pieces.append(jnp.swapaxes(er, 1, 2))
    if layout.extra_per_cell_count:
        extra = stage_stack[:, er_end:].reshape(
            (kernel_context.num_stages, layout.extra_per_cell_count, layout.n_radial)
        )
        pieces.append(jnp.swapaxes(extra, 1, 2))
    per_stage_blocks = jnp.concatenate(pieces, axis=2)
    return jnp.swapaxes(per_stage_blocks, 0, 1).reshape(
        (layout.n_radial, int(kernel_context.num_stages) * layout.variables_per_cell)
    )


def _radau_radial_blocks_to_stage_stack(
    kernel_context: _RadauAcceptedStepKernelContext,
    radial_blocks,
):
    """Convert radial block ordering back to ``(num_stages, state_dim)``."""
    layout = _radau_infer_radial_block_layout(kernel_context)
    radial_blocks = jnp.asarray(radial_blocks, dtype=kernel_context.dtype).reshape(
        (layout.n_radial, int(kernel_context.num_stages), layout.variables_per_cell)
    )
    per_stage_blocks = jnp.swapaxes(radial_blocks, 0, 1)
    cursor = 0
    pieces = []
    if layout.density_species_count:
        density = per_stage_blocks[:, :, cursor:cursor + layout.density_species_count]
        pieces.append(jnp.swapaxes(density, 1, 2).reshape((kernel_context.num_stages, -1)))
        cursor += layout.density_species_count
    if layout.pressure_species_count:
        pressure = per_stage_blocks[:, :, cursor:cursor + layout.pressure_species_count]
        pieces.append(jnp.swapaxes(pressure, 1, 2).reshape((kernel_context.num_stages, -1)))
        cursor += layout.pressure_species_count
    if layout.has_er:
        er = per_stage_blocks[:, :, cursor:cursor + 1]
        pieces.append(jnp.swapaxes(er, 1, 2).reshape((kernel_context.num_stages, -1)))
        cursor += 1
    if layout.extra_per_cell_count:
        extra = per_stage_blocks[:, :, cursor:cursor + layout.extra_per_cell_count]
        pieces.append(jnp.swapaxes(extra, 1, 2).reshape((kernel_context.num_stages, -1)))
...
def _radau_solve_exact_stage_residual_transpose_woodbury(
    ...
    *,
    ...
    """Solve the stage transpose system with block-tridiagonal Woodbury correction.

    This is an experimental non-iterative mode. It currently builds the exact
    stage residual matrix to obtain a truncated SVD of the non-tridiagonal
    radial coupling. That makes it useful for value/timing experiments, but not
    yet the final memory-optimized compact implementation.
    """
    ...
    # transpose solve therefore uses R.T, i.e. swaps U and V.
    u_transpose = v_rank
    v_transpose = u_rank
...
def _radau_solve_exact_stage_residual_transpose_compact(
    ...
    *,
    ...
    """Solve the exact stage transpose system using a non-dense compact hook."""
    solve_fn = physics_context.exact_stage_transpose_solve_compact
    if solve_fn is None:
        raise ValueError(
            "reverse_stage_adjoint_solve_mode='exact_block_compact' requires a "
            "non-dense exact_stage_transpose_solve_compact hook. The dense block "
            "fallback is intentionally disabled so this mode cannot accidentally "
            "materialize the full Radau stage Jacobian."
        )
    return solve_fn(
        kernel_context,
        physics_context,
...
def _build_prepared_radau_accepted_rollout(
    *,
    ...
    flat_rhs_build_support_pullback_batched_interpolated_faces_reuse_local_vjp_primal = (
        _flat_rhs_build_support_pullback_batched_interpolated_faces_factory(
            unravel=unpack_flat, vector_field=vector_field, args=args, kwargs=kwargs,
            project_flat=project_flat, reuse_local_vjp_primal=True,
        )
    )
    flat_rhs_build_support_pullback_batched_interpolated_faces_multi_rhs_shared_primal = (
        _flat_rhs_build_support_pullback_batched_interpolated_faces_factory(
            unravel=unpack_flat, vector_field=vector_field, args=args, kwargs=kwargs,
            project_flat=project_flat, multi_rhs_shared_primal=True,
        )
    )
    ...
    flat_rhs_build_support_pullback_batched_interpolated_faces_native_multi_rhs_reuse_moment_drds_jvp_shared_primal = (
        _flat_rhs_build_support_pullback_batched_interpolated_faces_factory(
            unravel=unpack_flat, vector_field=vector_field, args=args, kwargs=kwargs,
            project_flat=project_flat, native_multi_rhs_reuse_moment_drds_jvp_shared_primal=True,
        )
    )
    flat_rhs_build_support_pullback_batched_interpolated_faces_native_multi_rhs_reuse_moment_drds_jvp_shared_primal_with_vmec_coefficients = (
        _flat_rhs_build_support_pullback_batched_interpolated_faces_factory(
            unravel=unpack_flat, vector_field=vector_field, args=args, kwargs=kwargs,
            project_flat=project_flat,
    ...
    flat_rhs_build_support_pullback_batched_interpolated_faces_native_multi_rhs_reuse_moment_drds_jvp_shared_primal_with_vmec_coefficients_direct_directional_product_rule = (
        _flat_rhs_build_support_pullback_batched_interpolated_faces_factory(
            unravel=unpack_flat, vector_field=vector_field, args=args, kwargs=kwargs,
            project_flat=project_flat,
            native_multi_rhs_reuse_moment_drds_jvp_shared_primal_with_vmec_coefficients_direct_directional_product_rule=True,
        )
    )
    ...
    flat_rhs_build_state_and_support_pullback_batched_interpolated_faces_native_multi_rhs_reuse_moment_drds_jvp_shared_primal = (
        _flat_rhs_build_state_and_support_pullback_batched_interpolated_faces_factory(
            unravel=unpack_flat,
            pack_flat=pack_state,
            vector_field=vector_field,
    ...
    flat_rhs_flux_state_pullback_generic = _flat_rhs_split_state_pullback_factory(
        unravel=unpack_flat,
        pack_flat=pack_state,
        vector_field=vector_field,
        args=args,
        kwargs=kwargs,
        project_flat=project_flat,
        component="flux_generic",
    )
    flat_rhs_joint_generic_state_pullback = _flat_rhs_split_state_pullback_factory(
        unravel=unpack_flat,
        pack_flat=pack_state,
        vector_field=vector_field,
        args=args,
    ...
    if reverse_stage_adjoint_solve_mode == "exact_block_compact" and exact_stage_transpose_solve_compact is None:
        raise ValueError(
    ...
    physics_context = _RadauAcceptedStepPhysicsContext(
        ...
        reverse_stage_adjoint_iter_maxiter=int(getattr(solver, "reverse_stage_adjoint_iter_maxiter", 40)),
        reverse_stage_adjoint_iter_tol=float(getattr(solver, "reverse_stage_adjoint_iter_tol", 1.0e-10)),
        reverse_stage_adjoint_woodbury_rank=int(getattr(solver, "reverse_stage_adjoint_woodbury_rank", 24)),
    )
...
class RADAUSolver(_RadauSolverConfig):
    def solve(self, state, vector_field: Callable, *args, **kwargs):
        ...
        physics_context = _RadauAcceptedStepPhysicsContext(
            ...
            reverse_stage_adjoint_iter_maxiter=int(getattr(self, "reverse_stage_adjoint_iter_maxiter", 40)),
            reverse_stage_adjoint_iter_tol=float(getattr(self, "reverse_stage_adjoint_iter_tol", 1.0e-10)),
            reverse_stage_adjoint_woodbury_rank=int(getattr(self, "reverse_stage_adjoint_woodbury_rank", 24)),
        )
