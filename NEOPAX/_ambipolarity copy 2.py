...
def find_all_ambipolar_Er_roots_min_entropy_jit(
    ...
    """Adaptive, padded, JIT-friendly ambipolar root search.
    1. Coarse scan to bracket roots.
    2. Refine up to max_roots roots per call using Newton-bisection.
    3. Pad with NaN if fewer roots; ignore extra roots.
    """
    er_min, er_max = float(Er_range[0]), float(Er_range[1])
    ...
    # Indices of valid brackets (pad with -1 if fewer than max_roots)
    valid_idxs = jnp.where(valid)[0]
    n_pad = max_roots - valid_idxs.shape[0]
    valid_idxs = jnp.pad(valid_idxs, (0, jnp.maximum(0, n_pad)), constant_values=-1)[:max_roots]

    def refine_bracket(idx):
        # If idx == -1, return NaN (no root)
        def _no_root():
        ...
        def _refine():
            ...
            entropy = entropy_func(x_final)
            return x_final, entropy
        return lax.cond(idx == -1, _no_root, _refine)

    # Vectorize refinement over up to max_roots brackets
    roots, entropies = jax.vmap(refine_bracket)(valid_idxs)
    n_roots = jnp.sum(valid)
    # Best root: minimum entropy among valid (ignore NaNs)
    entropy_masked = jnp.where(jnp.isfinite(entropies), entropies, jnp.inf)
    best_idx = jnp.argmin(entropy_masked)
    best_root = lax.cond(jnp.any(jnp.isfinite(entropies)), lambda _: roots[best_idx], lambda _: jnp.asarray(0.0, dtype=jnp.float64), None)
